...
const PRIMARY_ID_FIELDS = Object.freeze({
  AnalyticsEvent: "analytics_event_id",
  MatterProfitability: "matter_profitability_id",
  ClientProfitability: "client_profitability_id",
  EmployeeUtilization: "employee_utilization_id",
  RealizationMetric: "realization_metric_id",
  AnalyticsDashboard: "dashboard_id",
  AnalyticsExport: "analytics_export_id",
  ReadModelRefreshRun: "refresh_run_id",
});
...
function normalizeRecord(input = {}) {
  ...
  return Object.freeze({
    ...
    g8_runtime_readiness_claim: "runtime_write_ready",
...
function emptyState() {
  return { migrations: ["analytics-runtime-001-file-store"], records: [], idempotency: [], audit_events: [] };
}
...
export function createAnalyticsRepository({ filePath, seedRecords = [], preserveSeedRecords = false } = {}) {
  let closed = false;
  ...
  function assertOpen() {
    if (closed) throw new Error("Analytics repository is closed");
  }
  ...
  const repository = {
    ...
    recordIdempotency(entry = {}) {
      ...
      const value = Object.freeze({ tenant_id: entry.tenant_id, idempotency_key: entry.idempotency_key, operation: entry.operation ?? "analytics_operation", response: clone(entry.response ?? {}), created_at: entry.created_at ?? new Date().toISOString() });
