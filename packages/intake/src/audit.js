...
export function appendIntakeAuditEvent({ repository, event } = {}) {
  ...
  const eventId =
    event.event_id ??
    `intake:${event.action}:${event.tenant_id}:${event.object_type}:${event.object_id}:${event.idempotency_key ?? "single"}`;
