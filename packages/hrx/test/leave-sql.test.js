...
function setup({ failureInjector } = {}) {
  ...
  return {
    ...
    ledger,
    requestStore,
    service: createLeaveRequestService({
      store: requestStore,
...
test("legacy SQL approval rolls request state back when ledger phase fails", async () => {
  const { store, ledger, requestStore, service } = setup({
    failureInjector(stage) {
      if (stage === "legacy.approve.after_state") throw new Error("synthetic-ledger-phase-failure");
    },
  });
  ledger.append({
    tenant_id: "tenant-a",
    entry_id: "earned-rollback-001",
    employee_id: "emp-001",
    policy_id: "pto-us",
    entry_type: "earned",
    amount: 16,
    occurred_on: "2026-06-20",
    source_ref: "PolicyAccrual:rollback",
  });
  await service.submit(
    { tenant_id: "tenant-a", actor_id: "user-a" },
    {
      request_id: "leave-rollback-001",
      employee_id: "emp-001",
      policy_id: "pto-us",
      leave_type: "pto",
      amount: 8,
  ...
  await assert.rejects(
    service.approve({ tenant_id: "tenant-a", actor_id: "manager-a" }, { request_id: "leave-rollback-001" }),
    /synthetic-ledger-phase-failure/,
  );
  assert.equal(requestStore.get({ tenant_id: "tenant-a", request_id: "leave-rollback-001" }).state, "submitted");
  assert.equal(ledger.balance({ tenant_id: "tenant-a", employee_id: "emp-001", policy_id: "pto-us" }).used_balance, 0);
  store.close();
});
