...
test("HRX domain inventory covers migrations, tables, append-only, CAS, payroll and PII names", () => {
  ...
  try {
    ...
    assert.equal(result.inventory.migration_count, 29);
    assert.equal(result.inventory.expected_migration_count, 29);
    assert.equal(result.inventory.table_count, 73);
    assert.equal(result.inventory.append_only_table_count, 10);
    assert.equal(result.inventory.cas_table_count, 23);
    ...
    assert.equal(Object.keys(result.inventory.table_hashes).length, 73);
...
test("HRX PostgreSQL import, async store port, legacy service command, CAS and append-only contracts pass", async (t) => {
  ...
  await port.transaction({ tenant_id: TENANT }, async (tx) => {
  ...
  await flushHrxStoreToPostgres({ ledger, store: first, tenant_id: TENANT });
  ...
  await assert.rejects(
    flushHrxStoreToPostgres({ ledger, store: second, tenant_id: TENANT }),
  ...
  const rehearsal = await ledger.recordRehearsal({
    ...
    smoke_result: {
      ...
      migration_count: source.inventory.migration_count,
      source_import_equal: true,
      legacy_service_command_equal: legacyCommand.flush.comparison.equal,
      async_transaction_passed: true,
      payroll_rows_preserved: true,
