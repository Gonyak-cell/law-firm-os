...
test("MG-004 executes migrations 001-029 on a fresh SQLite database", () => {
  ...
  assert.equal(report.migration_last, "029_hrx_leave_accrual_rule_version_index.sql");
  assert.deepEqual(
    report.migration_receipts.map((receipt) => receipt.filename.slice(0, 3)),
    Array.from({ length: 29 }, (_, index) => String(index + 1).padStart(3, "0")),
  );
  assert.deepEqual(report.actual_object_counts, { tables: 73, indexes: 54, triggers: 12 });
