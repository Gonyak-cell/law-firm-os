...
test("MG-004 executes migrations 001-048 on a fresh SQLite database", () => {
  const report = auditFreshHrxDatabase();
  assert.equal(report.verdict, "PASS");
  assert.equal(report.migration_count, 48);
  assert.equal(report.migration_first, "001_hrx_core.sql");
  assert.equal(report.migration_last, "048_hrx_recruiting_pipeline_receipts.sql");
  assert.deepEqual(
    report.migration_receipts.map((receipt) => receipt.filename.slice(0, 3)),
    Array.from({ length: 48 }, (_, index) => String(index + 1).padStart(3, "0")),
  );
  assert.deepEqual(report.actual_object_counts, { tables: 83, indexes: 75, triggers: 24 });
