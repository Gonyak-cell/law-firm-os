...
test("MG-004 executes migrations 001-028 on a fresh SQLite database", () => {
  const report = auditFreshHrxDatabase();
  assert.equal(report.verdict, "PASS");
  assert.equal(report.migration_count, 28);
  assert.equal(report.migration_first, "001_hrx_core.sql");
  assert.equal(report.migration_last, "028_hrx_leave_accrual_rule_versions.sql");
  assert.deepEqual(
    report.migration_receipts.map((receipt) => receipt.filename.slice(0, 3)),
    Array.from({ length: 28 }, (_, index) => String(index + 1).padStart(3, "0")),
  );
  assert.deepEqual(report.actual_object_counts, { tables: 73, indexes: 53, triggers: 12 });
  ...
  assert.equal(report.empty_table_count, 73);
