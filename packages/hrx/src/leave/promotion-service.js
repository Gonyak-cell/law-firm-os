...
export function createLeavePromotionService({ store, documents, employeeDirectory = () => [], clock = () => new Date().toISOString(), idFactory = (prefix) => `${prefix}_${randomUUID()}` } = {}) {
  ...
  function preview(context, input = {}) {
    ...
    const employees = new Map(employeeDirectory({ tenant_id: tenantId }).filter((employee) => allowed.has(employee.employee_id) && !["inactive", "terminated", "archived"].includes(employee.status)).map((employee) => [employee.employee_id, employee]));
    ...
    const sourceVersion = hash({ policy_version_id: policyVersionId, reference_date: referenceDate, balance_source_version: balance.source_version, targets: targets.map((row) => [row.employee_id, row.available_minutes, row.source_version]) });
    return Object.freeze({
      ...
      calculation_snapshot_hash: hash({ legalSchedule, targets, sourceVersion }),
  ...
  function list(context) {
    ...
    return Object.freeze(store.query("select", { table: "hrx_leave_promotion_campaigns", where: { tenant_id: tenantId } })
      ...
      .map((campaign) => campaignView(campaign, byCampaign.get(campaign.campaign_id).sort((left, right) => String(left.employee_display_name).localeCompare(String(right.employee_display_name), "ko")))));
  }
  ...
  function issueNotice(context, recipientId, stage, input = {}) {
    ...
    if (recipient[documentField]) {
      ensurePromotionOutbox(store, {
        ...
        payload: { recipient_id: recipientId, campaign_id: recipient.campaign_id, stage },
    ...
    documents.create({
      ...
      source_metadata: { provider_document_id: documentId, campaign_id: recipient.campaign_id, recipient_id: recipient.recipient_id, notice_stage: stage, delivery_state: "pending", etag_present: false, web_url_present: false },
      title: stage === "first" ? "연차휴가 사용 시기 지정 촉구서" : "연차휴가 사용 시기 지정 통보서",
    });
    ...
    const patch = stage === "first"
      ? { document_id: documentId, first_document_version: documentVersion, first_issued_at: now, first_delivery_state: "pending", state: "first_notice_issued", stage: "first_notice", deadline_at: recipient.first_notice_deadline_at, late_reasons_json: JSON.stringify([...new Set(lateReasons)]), updated_at: now }
      : { second_document_id: documentId, second_document_version: documentVersion, second_issued_at: now, second_delivery_state: "pending", state: "second_notice_issued", stage: "second_notice", deadline_at: recipient.second_notice_deadline_at, late_reasons_json: JSON.stringify([...new Set(lateReasons)]), updated_at: now };
    ...
    appendAudit(context, `hrx.leave.promotion.${stage}_notice.issue`, "LeavePromotionRecipient", recipientId, `leave_promotion_${stage}_notice_document_created`, { document_id: documentId, document_version: documentVersion, document_body_included: false });
    return recipientView(updated);
  }
  ...
  function recordEvidence(context, recipientId, input = {}) {
    ...
    const digest = evidenceHash(input.evidence_hash);
    ...
    const lateReasons = parseJson(recipient.late_reasons_json, []);
    const deadlineAt = stage === "first" ? recipient.first_notice_deadline_at : recipient.second_notice_deadline_at;
    if (deadlineAt && occurredAt > deadlineAt) lateReasons.push(`${stage}_notice_late`);
    const patch = { late_reasons_json: JSON.stringify([...new Set(lateReasons)]), updated_at: clock() };
    if (stage === "first") {
      if (eventType === "delivered") {
        ...
        Object.assign(patch, { first_delivery_state: "delivered", first_delivered_at: occurredAt, first_evidence_hash: digest, delivery_evidence_hash: digest, response_due_at: new Date(Date.parse(occurredAt) + responseDays * 86_400_000).toISOString(), state: "awaiting_employee_response" });
      }
      if (eventType === "viewed") Object.assign(patch, { first_viewed_at: occurredAt });
      if (eventType === "failed") Object.assign(patch, { first_delivery_state: "failed", first_evidence_hash: digest, delivery_evidence_hash: digest, state: "first_delivery_failed" });
    } else {
      if (eventType === "delivered") Object.assign(patch, { second_delivery_state: "delivered", second_delivered_at: occurredAt, second_evidence_hash: digest, delivery_evidence_hash: digest, state: "second_notice_delivered", compliance_state: "delivery_verified_view_pending" });
      if (eventType === "viewed") Object.assign(patch, { second_viewed_at: occurredAt, state: "second_notice_viewed", compliance_state: "evidence_complete_pending_legal_review" });
      if (eventType === "failed") Object.assign(patch, { second_delivery_state: "failed", second_evidence_hash: digest, delivery_evidence_hash: digest, state: "second_delivery_failed" });
    }
    const updated = store.query("updateOne", { table: "hrx_leave_promotion_recipients", where: { tenant_id: tenantId, recipient_id: recipientId }, patch });
    if (eventType === "delivered") {
      ...
      documents.update({ tenant_id: tenantId, document_id: documentId }, { source_status: "verified", source_verified_at: occurredAt, source_metadata: { ...document.source_metadata, provider_document_id: providerReceiptRef, delivery_state: "delivered", view_state: "pending", evidence_hash_present: true, etag_present: false, web_url_present: false } });
    }
    if (eventType === "viewed") {
      const document = documents.get({ tenant_id: tenantId, document_id: documentId });
      documents.update({ tenant_id: tenantId, document_id: documentId }, { source_metadata: { ...document.source_metadata, view_state: "viewed", viewed_at_present: true } });
    }
    appendAudit(context, `hrx.leave.promotion.${stage}_notice.${eventType}`, "LeavePromotionRecipient", recipientId, `leave_promotion_${stage}_notice_${eventType}_evidence_recorded`, { document_id: documentId, evidence_hash: digest, provider_receipt_ref: input.provider_receipt_ref ?? null });
    return recipientView(updated);
  }
