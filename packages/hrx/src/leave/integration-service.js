...
function payloadFor(store, outbox, providerKind) {
  ...
  if (providerKind === "notification") {
    const recipientSeed = context?.request.employee_id ?? outbox.aggregate_id;
    return Object.freeze({
  ...
  if (providerKind === "schedule") {
    return Object.freeze({
      ...
      operation: isCancellation ? "cancel" : "upsert",
...
function deliveryView(row) {
  return Object.freeze({ ...row, payload: Object.freeze(parseJson(row.payload_json, {})), payload_json: undefined });
}

export function createInternalLeaveIntegrationProviders() {
  const receipts = new Map();
  return Object.freeze(Object.fromEntries(PROVIDER_KINDS.map((providerKind) => [providerKind, Object.freeze({
...
export function createLeaveIntegrationService({
  ...
  async function deliver(context, outbox, current) {
    ...
    try {
      ...
      const providerReceiptRef = requiredString(result, "provider_receipt_ref");
  ...
  function list(context, { limit = 100 } = {}) {
    ...
    const rows = outbox.map((event) => Object.freeze({
      ...event,
      payload_json: undefined,
      deliveries: Object.freeze(deliveries.filter((delivery) => delivery.outbox_event_id === event.outbox_event_id).map(deliveryView)),
    }));
    const summary = {
  ...
  return Object.freeze({ list, process });
}
