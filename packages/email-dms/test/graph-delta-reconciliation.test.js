...
const CURSOR_CODEC = createGraphCursorCodec({ key: Buffer.alloc(32, 7), random_bytes: () => Buffer.alloc(12, 3) });
...
test("OUTM-27 advances Graph delta pages only after durable enqueue and resumes from the stored cursor", async () => {
  ...
  const service = createGraphDeltaReconciliationService({ repository, cursor_codec: CURSOR_CODEC, queue, provider, clock: () => new Date("2026-08-08T00:00:00.000Z"), recovery_window_ms: 60 * 60 * 1000 });
  ...
  assert.equal(JSON.stringify(repository.snapshot()).includes("$deltatoken=done"), false);
  assert.equal(CURSOR_CODEC.open(repository.snapshot().cursors[0].cursor_ref, { tenant_id: PRINCIPAL.tenant_id, m365_connection_id: PRINCIPAL.m365_connection_id, resource: RESOURCE }).includes("$deltatoken=done"), true);
...
test("OUTM-27 clears an expired delta cursor and performs one bounded full reconciliation", async () => {
  ...
  repository.transaction((state) => state.cursors.push({
    ...
    m365_connection_id: "connection-outm27",
    resource: RESOURCE,
    cursor_ref: CURSOR_CODEC.seal("https://graph.microsoft.com/v1.0/me/mailFolders('inbox')/messages/delta?$deltatoken=expired", { tenant_id: PRINCIPAL.tenant_id, m365_connection_id: PRINCIPAL.m365_connection_id, resource: RESOURCE }),
    version: 1,
  }));
  const queue = createGraphNotificationQueue({ repository });
  const calls = [];
  ...
  // When
  const result = await createGraphDeltaReconciliationService({ repository, cursor_codec: CURSOR_CODEC, queue, provider, clock: () => new Date("2026-08-08T00:00:00.000Z"), recovery_window_ms: 15 * 60 * 1000 }).reconcile({ ...PRINCIPAL, resources: [RESOURCE] });
