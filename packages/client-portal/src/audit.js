...
export function appendPortalAuditEvent({ repository, event } = {}) {
  ...
  const eventId =
    event.event_id ??
    `portal:${event.action}:${event.tenant_id}:${event.object_type}:${event.object_id}:${event.idempotency_key ?? "single"}`;
