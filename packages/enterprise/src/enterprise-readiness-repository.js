...
const PRIMARY_ID_FIELDS = Object.freeze({
  EnterpriseReadinessItem: "enterprise_item_id",
  ReleaseCandidate: "release_candidate_id",
  GoNoGoDecision: "go_no_go_id",
});
...
function normalizeRecord(input = {}) {
  ...
  return Object.freeze({
    ...
    owner_module: "enterprise-readiness",
    ...
    dispatches_enterprise_readiness_runtime: true,
    owner_decision_required: input.owner_decision_required ?? true,
    release_gate_required: input.release_gate_required ?? true,
...
function emptyState() {
  return { migrations: ["enterprise-readiness-runtime-001-file-store"], records: [], idempotency: [], audit_events: [] };
}
...
export function createEnterpriseReadinessRepository({ filePath, seedRecords = [] } = {}) {
  ...
  function assertOpen() {
    if (closed) throw new Error("Enterprise readiness repository is closed");
  }
  ...
  const repository = {
    ...
    list(query = {}) {
      ...
      return Object.freeze(
        [...records.values()]
          ...
          .filter((record) => !query.item_type || record.item_type === query.item_type)
    ...
    recordIdempotency(entry = {}) {
      ...
      const value = Object.freeze({ tenant_id: entry.tenant_id, idempotency_key: entry.idempotency_key, operation: entry.operation ?? "enterprise_readiness_operation", response: clone(entry.response ?? {}), created_at: entry.created_at ?? new Date().toISOString() });
