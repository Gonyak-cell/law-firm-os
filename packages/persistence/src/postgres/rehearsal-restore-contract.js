...
export const JSON_POSTGRES_REHEARSAL_RESTORE_TARGET_VERSION =
  "law-firm-os.json-postgres-rehearsal-restore-target.v1";
...
function material(value) {
  return Object.fromEntries(
    KEYS
      .filter((key) => key !== "restore_target_sha256")
...
function digest(value) {
  return createHash("sha256")
    .update(canonicalizeJson(material(value)))
    .digest("hex");
}

export function createJsonPostgresRehearsalRestoreTarget(input = {}, {
  ...
  const value = Object.fromEntries(
    KEYS
      .filter((key) =>
        !["schema_version", "restore_target_sha256"].includes(key))
      .map((key) => [key, input[key]]),
  );
  value.schema_version =
    JSON_POSTGRES_REHEARSAL_RESTORE_TARGET_VERSION;
  value.performance_acceptance_sha256 =
    acceptance.acceptance_sha256;
  value.restore_target_sha256 = digest(value);
  validateJsonPostgresRehearsalRestoreTarget(value, {
    performanceAcceptance,
  });
  return Object.freeze(value);
}

export function validateJsonPostgresRehearsalRestoreTarget(value = {}, {
  sourceSha,
  sourceTree,
  packetSha256,
  performanceAcceptance,
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)
    ...
    || value.source_database_identifier
      !== "lawos-private-staging-postgres"
    || !value.restore_database_identifier.startsWith(
      `lawos-private-rehearsal-restore-${value.source_sha.slice(0, 10)}-`,
    )
    || value.aws_account !== "770880870480"
    || value.aws_region !== "ap-northeast-2"
    || !new RegExp(
      `^[a-z0-9-]+\\.[a-z0-9.-]+\\.${value.aws_region}`
      + "\\.rds\\.amazonaws\\.com$",
      "u",
    ).test(value.endpoint_address ?? "")
    || value.endpoint_port !== 5432
    || value.database_name !== "lawos_rehearsal"
    || ![
      value.source_latest_restorable_at,
      value.restore_started_at,
      value.restore_available_at,
    ].every((time) =>
      TIME.test(time ?? "") && Number.isFinite(Date.parse(time)))
    || Date.parse(value.source_latest_restorable_at)
      > Date.parse(value.restore_started_at)
    || Date.parse(value.restore_available_at)
      < Date.parse(value.restore_started_at)
    ...
    || [
      value.vpc_sha256,
      value.subnet_group_sha256,
      value.security_group_set_sha256,
      value.kms_key_arn_sha256,
    ...
    || value.restore_target_sha256 !== digest(value)) {
    fail(
      "W12 restore target schema, binding, isolation, objective, or digest is invalid",
    );
  }
  if ((sourceSha && value.source_sha !== sourceSha)
    ...
    fail("W12 restore target source or packet binding drifted");
  }
