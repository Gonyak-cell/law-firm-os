...
const PRIMARY_ID_FIELDS = Object.freeze({
  ...
  CriticalPathRun: "critical_path_run_id",
  UiAdjudication: "ui_adjudication_id",
});
...
function normalizeRecord(input = {}) {
  ...
  return Object.freeze({
    ...
    owner_module: "platform-ui-readiness",
    ...
    dispatches_ui_runtime: true,
    api_backed_surface: input.api_backed_surface ?? true,
    permission_states_covered: input.permission_states_covered ?? true,
    review_states_covered: input.review_states_covered ?? true,
    responsive_states_covered: input.responsive_states_covered ?? true,
...
export function createUiReadinessRepository({ filePath, seedRecords = [], preserveSeedRecords = false } = {}) {
  let closed = false;
  ...
  const repository = {
    ...
    list(query = {}) {
      ...
      return Object.freeze(
        [...records.values()]
          ...
          .filter((record) => !query.route_id || record.route_id === query.route_id)
    ...
    recordIdempotency(entry = {}) {
      ...
      const value = Object.freeze({ tenant_id: entry.tenant_id, idempotency_key: entry.idempotency_key, operation: entry.operation ?? "ui_readiness_operation", response: clone(entry.response ?? {}), created_at: entry.created_at ?? new Date().toISOString() });
