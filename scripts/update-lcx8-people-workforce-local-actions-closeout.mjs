...
const PROOF_JSON = `${ARTIFACT_DIR}/lcx8-action-0183-0196-people-workforce-local-actions-proof.json`;
const PROOF_MD = `${ARTIFACT_DIR}/lcx8-action-0183-0196-people-workforce-local-actions-proof.md`;
const CLOSEOUT_JSON = `${ARTIFACT_DIR}/lcx8-post-closeout-people-workforce-local-actions-2026-06-28.json`;
const CLOSEOUT_MD = `${ARTIFACT_DIR}/lcx8-post-closeout-people-workforce-local-actions-2026-06-28.md`;
...
for (const id of ROW_IDS) {
  ...
  appendUnique(row.evidence, `Post-closeout ${id} People workforce proof: clicked ${row.label}; observed "${proofRow.observed_text}"; artifact=${PROOF_JSON}`);
  appendUnique(row.evidence, "Post-closeout non-claim: local UI-state/API-read proof only; no persistence, API write, external receipt, production-ready, or go-live claim.");
  row.status = "PASS";
  row.remediation_lane = "resolved";
  row.trace_depth = proofRow.trace_depth;
  if (id === "LCX8-ACTION-0195") {
    row.api_route = "/api/hrx/employees/:employeeId";
    row.handler = "onClick -> onSelectEmployee(row.employeeId) -> EmployeeProfile api read";
  } else if (id === "LCX8-ACTION-0196") {
    row.api_route = "/api/hrx/lifecycle/onboarding; /api/hrx/lifecycle/offboarding";
    row.handler = "onClick -> local lifecycle selection state; PeopleHome renders LifecycleBoard api read";
  } else {
    row.handler = "onClick -> PeopleWorkforceDirectory localAction state panel";
  }
  if (!String(row.notes ?? "").includes("People workforce local-action proof")) {
...
const resolution = {
  schema_version: "law-firm-os.lcx8.post-closeout.people-workforce-local-actions.v0.1",
  ...
  source_fixes: [
    "apps/web/src/people/employees/PeopleWorkforceDirectory.tsx wires roster toolbar/header controls to visible local action state.",
    "apps/web/src/people/employees/PeopleWorkforceDirectory.tsx routes roster row clicks through explicit selection handling.",
    "apps/web/src/people/PeopleHome.tsx renders EmployeeProfile on people-members and LifecycleBoard on people-lifecycle."
  ],
  ...
  network: {
    hrx_reads: proof.hrxRequests.length - proof.hrxWrites.length,
    hrx_writes: proof.hrxWrites.length
  },
...
writeFileSync(CLOSEOUT_MD, `${[
  ...
  `- HRX reads: ${resolution.network.hrx_reads}`,
  `- HRX writes: ${resolution.network.hrx_writes}`,
  "",
...
for (const path of [ACTION_RUN, POST_STATUS_JSON, FINAL_LANE_JSON, RISK_JSON, ISSUES_JSON]) {
  ...
  if (path === FINAL_LANE_JSON) {
    for (const assignment of doc.assignments ?? []) {
      ...
      assignment.reason = "resolved_by_people_workforce_local_state_and_api_read_proof";
  ...
  if (path === RISK_JSON) {
    for (const ranked of [...(doc.ranked_rows ?? []), ...(doc.top_20_rows ?? [])]) {
      ...
      ranked.score = 0;
      ranked.reason = "Resolved by People workforce local UI-state/API-read proof.";
  ...
  if (path === ISSUES_JSON) {
    for (const issue of doc.issues ?? []) {
      ...
      issue.tier = "resolved";
      issue.score = 0;
      issue.title = `Resolved People workforce action for ${issue.label}`;
      issue.body = `Resolved by ${PROOF_JSON}. Browser proof clicked the visible control and observed local state or existing API-read panel behavior without persistence, API write, external receipt, production-ready, or go-live claim.`;
      issue.risk_reason = "Resolved by People workforce local UI-state/API-read proof.";
...
backlog = replaceBlock(
  ...
  [
    ...
    "",
    "- Latest resolved evidence: People workforce local-action proof promoted 7 Lane A rows to `PASS`.",
    ...
    `- Verification: ${verificationSummary}`,
    "- Non-claim: local UI-state/API-read proof only; no persistence, API write, external receipt, production-ready, or go-live claim is made."
  ].join("\n")
);
for (const id of ROW_IDS) {
  const proofBody = `Resolved by ${PROOF_JSON}. Browser proof clicked the visible control and observed local state or existing API-read panel behavior without persistence, API write, external receipt, production-ready, or go-live claim.`;
  const pattern = new RegExp(`^\\| (\\d+) \\| ${id} \\| [^|]+ \\| [^|]+ \\| [^|]+ \\| ([^|]+) \\| ([^|]+) \\| [^|]+ \\| [^|]+ \\|$`, "m");
  backlog = backlog.replace(pattern, `| $1 | ${id} | resolved | PASS | resolved | $2 | $3 | Resolved People workforce action | ${proofBody} |`);
}
...
if (!postStatus.includes(PROOF_JSON)) {
  postStatus += `\n- People workforce local-action closeout: ${PROOF_JSON}; ${CLOSEOUT_JSON}.\n`;
}
...
if (!audit.includes("## Post-Closeout People Workforce Local Actions Proof")) audit += `${auditSection}\n`;
