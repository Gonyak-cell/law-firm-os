...
const planPath = "docs/desktop/mater-desktop-temporary-release-plan.md";
const receiptPath = "docs/desktop/mater-desktop-temporary-release-receipt.md";
...
const requiredPlanPhrases = [
  ...
  "Do not use `api.mater.example.com` unless that exact domain is owned and delegated.",
  "npm --workspace apps/desktop run test:smoke",
  "npm --workspace apps/desktop run test:file-bridge",
  "npm run mater-desktop:aws-runtime:smoke",
  ...
  "node scripts/validate-mater-desktop-temporary-release-plan.mjs",
  "docs/desktop/mater-desktop-temporary-release-receipt.md",
  "node scripts/release-mater-desktop-temporary.mjs",
  "apps/desktop/dist/release/mater-desktop-internal-0.1.0/release-manifest.json",
  "node scripts/validate-mater-desktop-temporary-release-bundle.mjs",
...
const requiredReceiptPhrases = [
  ...
  "`npm run mater-desktop:aws-runtime:smoke` | PASS",
...
assert.match(pkg.description, /mater desktop shell package/, "desktop description must identify mater shell");
assert.match(builder, /^appId:\s*com\.amic\.mater\.desktop\.internal$/m, "builder appId must remain internal");
assert.match(builder, /^productName:\s*mater$/m, "builder productName must be mater");
assert.match(builder, /^artifactName:\s*"mater-internal-\$\{version\}-\$\{os\}-\$\{arch\}\.\$\{ext\}"$/m, "builder artifactName must remain internal");
