...
assert(contract.runtime_admission_gate?.storage_decision_resolved === false, "storage decision must remain unresolved");
assert(contract.runtime_admission_gate?.privilege_classification_decision_resolved === false, "privilege decision must remain unresolved");
...
for (const item of contract.storage_dependency_partition?.storage_dependent ?? []) {
  assert(item.blocked_until_storage_decision === true, `${item.id} must be blocked until storage decision`);
}
