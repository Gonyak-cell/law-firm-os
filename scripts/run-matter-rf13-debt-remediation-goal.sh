...
block() {
  /usr/bin/printf '%s\n' '{"validator":"matter-rf13-debt-remediation-goal","verdict":"BLOCKED_BY_AUTHORITY","code":"RF13_GOAL_OPERATIONAL_LAUNCHER_REQUIRED"}' >&2
  exit 2
}
...
launcher_path=${0:A}
runner_path=${launcher_path:h}/internal/run-matter-rf13-debt-remediation-goal.mjs
...
umask 077
attestation_dir=$(/usr/bin/mktemp -d "${TMPDIR:-/tmp}/matter-rf13-goal-launch.XXXXXX")
