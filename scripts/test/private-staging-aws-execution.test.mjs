...
test("deployed Lambda, RDS, and DMS bucket contracts fail closed", () => {
  ...
  assert.equal(assertPrivateStagingBucket({ versioning: { Status: "Enabled" }, publicAccess: { PublicAccessBlockConfiguration: { BlockPublicAcls: true, IgnorePublicAcls: true, BlockPublicPolicy: true, RestrictPublicBuckets: true } }, encryption: { ServerSideEncryptionConfiguration: [{ ApplyServerSideEncryptionByDefault: { SSEAlgorithm: "aws:kms" } }] }, objectLock: { ObjectLockConfiguration: { ObjectLockEnabled: "Enabled" } } }).public_bucket_count, 0);
  assert.throws(() => assertPrivateStagingBucket({ versioning: { Status: "Enabled" }, publicAccess: { PublicAccessBlockConfiguration: { BlockPublicAcls: true, IgnorePublicAcls: true, BlockPublicPolicy: true, RestrictPublicBuckets: true } }, encryption: { ServerSideEncryptionConfiguration: [{ ApplyServerSideEncryptionByDefault: { SSEAlgorithm: "aws:kms" } }] }, objectLock: { ObjectLockEnabled: "Enabled" } }), /Object Lock/u);
});
...
test("admin events and CUT-005/CUT-006 results require exact bindings and zero counters", () => {
  ...
  const cut005 = { ...common, action: "lawos-private-staging-cut-005", source_record_count: 21, accepted_record_count: 14, rejected_row_count: 7, unexpected_rejection_count: 0, shadow_difference_count: 0, tenant_negative_visible_count: 0, json_fallback_count: 0, json_writer_count: 0, dual_write_count: 0, transactional_rollback: { residual_item_count: 0 }, resume_equivalence: { resume_equal: true, immediate_replay_noop: true } };
...
test("execution receipt builder populates every strict W11 field", () => {
  const value = buildPrivateStagingExecutionReceipt({
    ...
    safeCounts: { zero_counter_count: 6, password_reset_count: 1, real_data_count: 0 }, digests: { cut_result_sha256: "e".repeat(64) }, claims: { cut_006_executed: true, internal_password_authority_verified: true },
  });
  ...
  assert.equal(value.claims.internal_identity_authority_verified, true);
  assert.equal(Object.hasOwn(value.claims, "internal_password_authority_verified"), false);
});
