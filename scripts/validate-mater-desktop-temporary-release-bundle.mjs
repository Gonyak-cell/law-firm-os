...
const releaseId = `mater-desktop-internal-${desktopPackage.version}`;
...
assert.equal(manifest.schema_version, "law-firm-os.mater-desktop-temporary-release.v0.1");
assert.equal(manifest.release_id, releaseId);
assert.equal(manifest.status, "internal_temporary_release_executed");
assert.equal(manifest.product_name, "mater");
assert.equal(manifest.package_name, "@law-firm-os/desktop");
assert.equal(manifest.internal_app_id, "com.amic.mater.desktop.internal");
