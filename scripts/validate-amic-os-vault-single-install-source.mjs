...
export async function validateAmicOsVaultSingleInstallSource({
  ...
  if (!installerNshSource.includes("!macro customInstall")
      ...
      || !/!macro customUnInstall[\s\S]*\$\{If\} \$\{RunningX64\}\s+SetRegView 64\s+\$\{Else\}\s+SetRegView 32\s+\$\{EndIf\}\s+SetShellVarContext current\s+RMDir \/r "\$LOCALAPPDATA\\AMIC OS\\OutlookAttachments"\s+\$\{If\} \$installMode == "all"\s+SetShellVarContext all\s+\$\{EndIf\}\s+!macroend/u.test(installerNshSource)
