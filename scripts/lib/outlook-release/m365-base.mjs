...
export function m365CompletionMillis(value, name, validationCutoff) {
  const completed = utcMillis(value, name);
  if (completed > validationCutoff) throw new Error(`${name} occurs after the trusted validation cutoff`);
  return completed;
}
...
export function validateM365Envelope(receipt, options) {
  ...
  const validationCutoff = utcMillis(options.validationCutoffUtc, "M365 validation cutoff");
  ...
  return { ...profiles, prerequisites, validationCutoff };
}
