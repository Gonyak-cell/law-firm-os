...
function validateProof(entry, receipt, options) {
  ...
  assertObservedAt(entry.observed_at_utc, "M365 propagation observation");
  return loaded;
}

export function validateM365Propagation(receipt, options) {
  const keys = new Set();
  const loaded = [];
  for (const entry of receipt.propagation_observations ?? []) {
    ...
    keys.add(key);
    loaded.push(validateProof(entry, receipt, options));
  }
