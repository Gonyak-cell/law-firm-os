...
export function buildStaticDryRunPlan({ releaseReceipt, releaseContext, sourceLocations, contract, bucketRef }) {
  ...
  const profiles = contract.profiles.map((expected) => {
    ...
    const operations = inventory.map((entry) => {
      ...
      return { byte_size: entry.byte_size, sha256: entry.sha256, source_path: `${contract.build.root}/${entry.path}`, target_key: targetKey };
    });
    return {
...
export function validateStaticDryRunPlan(plan, { contract, releaseReceipt, releaseContext, sourceLocations }) {
  ...
  assertExactKeys(plan, [
    "allowed_claim", "blocked_claim", "bucket_ref", "candidate_inventory_sha256", "delete",
    "execution_performed", "invalidation_paths", "mode", "mutation_count", "package_lock_sha256",
    "profiles", "protected_prefixes", "schema_version", "source_sha", "source_tree", "status",
  ], "static deploy plan");
  const releaseBuild = validateReleaseCandidateReceipt(releaseReceipt, contract, releaseContext);
  if (plan.schema_version !== "amic-os.outlook-static-deploy-plan.v1"
    ...
    || plan.execution_performed !== false || plan.mutation_count !== 0 || plan.delete !== false
  ...
  for (const expected of contract.profiles) {
    ...
    assertExactKeys(profile, [
      ...
      "manifest_publish_mode", "manifest_ref", "manifest_sha256", "operations", "product_id", "profile",
    ...
    for (const [index, operation] of profile.operations.entries()) {
      ...
      if (operation.source_path !== `${contract.build.root}/${entry.path}` || operation.target_key !== targetKey
        || operation.sha256 !== entry.sha256 || operation.byte_size !== entry.byte_size || targetKeys.has(targetKey)
