...
function validateProof(evidence, receipt, options) {
  ...
  assertObservedAt(evidence.observed_at_utc, "real Outlook observation");
  return loaded;
}

export function validateM365Hosts(receipt, options) {
  const keys = new Set();
  const loaded = [];
  for (const evidence of receipt.host_evidence ?? []) {
    ...
    keys.add(key);
    loaded.push(validateProof(evidence, receipt, options));
  }
