...
export function validateForwardStaticRollbackContract(forward, releaseContract, rollbackManifestBytes) {
  ...
  if (rollback.profile !== matter.profile || rollback.product_id !== matter.product_id
    ...
    || rollback.manifest_version !== "1.3.0.6" || rollback.manifest_sha256 !== sha256(manifestBytes)
    ...
    throw new Error("1.3.0.6 forward rollback manifest identity or semantics drifted");
  }
  ...
  if (!matterProfile || !rollback.source_locations.every((location) => (
    ...
    throw new Error("1.3.0.6 forward rollback does not point to the prior Matter taskpane inventory");
  }
  return {
