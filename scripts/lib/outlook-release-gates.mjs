...
export function validateReleaseContract(contract) {
  ...
  if (contract.static_deploy?.target_prefix !== "addin/"
    || contract.static_deploy?.invalidation_path !== "/addin/*"
    ...
    throw new Error("static deployment must default to additive /addin dry-run");
  }
...
export function validateDependencyLicenses(packageLock, contract) {
  ...
  return { checked_package_count: Object.values(inventory).reduce((sum, count) => sum + count, 0), licenses: canonical(inventory) };
}
...
export function validateReleaseCandidateReceipt(receipt, contract) {
...
export function buildStaticDryRunPlan({ releaseReceipt, sourceLocations, contract, bucketRef }) {
  const releaseBuild = validateReleaseCandidateReceipt(releaseReceipt, contract);
  if (!/^[A-Z][A-Z0-9_]{2,63}$/u.test(bucketRef ?? "")) throw new Error("static bucket_ref must be symbolic");
  const normalizedInventory = releaseBuild.inventory;
  const prefix = contract.static_deploy.target_prefix;
  const protectedPrefixes = contract.static_deploy.protected_prefixes ?? [];
  const operations = normalizedInventory.map((entry) => {
    const file = assertSafeRelativePath(entry.path, "static artifact path");
    const targetKey = `${prefix}${file}`;
    if (file.endsWith(".xml") || protectedPrefixes.some((protectedPrefix) => targetKey.startsWith(protectedPrefix))) {
      throw new Error(`static dry-run attempted to overwrite a manifest object: ${targetKey}`);
    }
    return { source_path: `${contract.build.root}/${file}`, target_key: targetKey, sha256: entry.sha256, byte_size: entry.byte_size };
  });
  const coverage = Object.fromEntries(contract.profiles.map((profile) => {
    const locations = sourceLocations?.[profile.profile] ?? [];
    const parsed = locations.map((value) => new URL(value));
    if (parsed.some((url) => url.protocol !== "https:" || url.username || url.password)) {
      throw new Error(`${profile.profile} SourceLocation must be credential-free HTTPS`);
    }
    return [profile.profile, parsed.length > 0 && parsed.every((url) => url.pathname.startsWith(`/${prefix}`))];
  }));
  return {
    ...
    invalidation_paths: [contract.static_deploy.invalidation_path],
    operations,
    source_location_coverage: coverage,
    allowed_claim: "This is an additive object and invalidation plan only; no AWS operation was executed.",
    blocked_claim: "This is not static deployment, cache propagation, central deployment, or Outlook runtime evidence.",
  };
}

export function validateStaticDryRunPlan(plan, contract) {
  ...
  if (plan?.schema_version !== "amic-os.outlook-static-deploy-plan.v1"
    ...
    || plan.delete !== false
    || !GIT_OID.test(plan.source_sha ?? "")
    || !GIT_OID.test(plan.source_tree ?? "")
    || !SHA256.test(plan.package_lock_sha256 ?? "")
    || !/^[A-Z][A-Z0-9_]{2,63}$/u.test(plan.bucket_ref ?? "")
    || plan.target_prefix !== contract.static_deploy.target_prefix
    || JSON.stringify(plan.protected_prefixes) !== JSON.stringify(contract.static_deploy.protected_prefixes)
    || plan.invalidation_paths?.length !== 1
    || plan.invalidation_paths[0] !== contract.static_deploy.invalidation_path) {
    throw new Error("static deploy plan escaped the additive /addin dry-run boundary");
  }
  const protectedPrefixes = contract.static_deploy.protected_prefixes ?? [];
  const operations = plan.operations ?? [];
  if (!operations.length || new Set(operations.map(({ target_key }) => target_key)).size !== operations.length) {
    throw new Error("static deploy plan must contain unique additive objects");
  }
  const inventory = [];
  for (const operation of operations) {
    if (!operation.target_key?.startsWith(contract.static_deploy.target_prefix)
      || operation.target_key.endsWith(".xml")
      || protectedPrefixes.some((prefix) => operation.target_key.startsWith(prefix))) {
      throw new Error(`static deploy operation escaped scope or touched a manifest: ${operation.target_key}`);
    }
    const relative = assertSafeRelativePath(
      operation.target_key.slice(contract.static_deploy.target_prefix.length),
      "static deploy target path",
    );
    if (operation.source_path !== `${contract.build.root}/${relative}`) {
      throw new Error(`static deploy source/target mapping drifted: ${operation.target_key}`);
    }
    inventory.push({ path: relative, byte_size: operation.byte_size, sha256: operation.sha256 });
  }
  validateBuildInventories(inventory, inventory, contract);
  const expectedCoverageKeys = sorted(contract.profiles.map(({ profile }) => profile));
  assertEqual(sorted(Object.keys(plan.source_location_coverage ?? {})), expectedCoverageKeys, "static SourceLocation coverage profiles");
  if (Object.values(plan.source_location_coverage).some((covered) => typeof covered !== "boolean")) {
    throw new Error("static SourceLocation coverage values must be boolean");
  }
  return { operation_count: operations.length, mutation_count: 0, execution_performed: false };
}
...
export function validateM365ReleaseReceipt(receipt, {
  ...
  validateReleaseCandidateReceipt(releaseCandidate, contract);
  ...
  for (const name of contract.m365.required_prerequisites ?? []) {
    ...
    if (prerequisite.status === "verified"
      && (!SHA256.test(prerequisite.evidence_sha256 ?? "") || !requiredText(prerequisite.evidence_ref, `${name}.evidence_ref`))) {
      throw new Error(`M365 release prerequisite evidence is incomplete: ${name}`);
    }
    if (prerequisite.status === "pending" && (prerequisite.evidence_sha256 != null || prerequisite.evidence_ref != null)) {
  ...
  assertEqual(sorted(Object.keys(claims)), claimNames, "M365 claim names");
  ...
  for (const evidence of receipt.host_evidence ?? []) {
    ...
    if (!expected
      ...
      || requiredScenarios.some((scenario) => !evidence.scenarios?.includes(scenario))
