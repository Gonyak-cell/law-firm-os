...
const SAFE_AWS_CLI_SERVICE_ERRORS = new Set([
  "AccessDenied", "AccessDeniedException", "AuthorizationHeaderMalformed", "BadDigest",
  "ConditionalRequestConflict", "EntityTooLarge", "ExpiredToken", "InternalError",
  "InvalidArgument", "InvalidDigest", "InvalidRequest", "InvalidToken", "MalformedXML",
  "NoSuchBucket", "NoSuchKey", "NotImplemented", "PreconditionFailed", "RequestTimeout",
  "ServiceUnavailable", "SignatureDoesNotMatch", "SlowDown",
  "KMS.AccessDeniedException", "KMS.DisabledException", "KMS.InvalidStateException",
  "KMS.NotFoundException", "KMS.ThrottlingException",
]);
const SAFE_AWS_CLI_PROCESS_ERRORS = new Set([
  "ENOENT", "EACCES", "ETIMEDOUT", "ENOBUFS", "ERR_CHILD_PROCESS_STDIO_MAXBUFFER",
]);

function sanitizedAwsCliError(error, args) {
  ...
  const code = error instanceof SyntaxError ? "AWS_CLI_INVALID_JSON"
    : match?.[2] === operation && SAFE_AWS_CLI_SERVICE_ERRORS.has(match[1]) ? match[1]
      : SAFE_AWS_CLI_PROCESS_ERRORS.has(error?.code) ? error.code : "AWS_CLI_FAILED";
  return Object.assign(new Error(`AWS CLI ${args.slice(0, 2).join(" ")} failed (${code})`), { code });
}

export function createWindowsSignedArtifactAwsCliAdapter({
  ...
  const json = (args) => {
    try {
      ...
      throw sanitizedAwsCliError(error, args);
    }
  };
  return Object.freeze({
