...
assert.match(source, /^appId:\s*com\.amic\.mater\.desktop\.internal$/m, "packaging config must use internal app ID");
assert.match(source, /^productName:\s*mater$/m, "packaging config must use product name mater");
  ...
  JSON.stringify(
    {
      verdict: "PASS",
      config: configPath,
      app_id: "com.amic.mater.desktop.internal",
      product_name: "mater",
      icon: iconPath,
