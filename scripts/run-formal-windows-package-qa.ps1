...
function Block-FormalPackageLauncher {
  [Console]::Error.WriteLine('{"verdict":"BLOCKED","code":"LAUNCHER_REQUIRED"}')
  exit 2
}

if ($UnexpectedArguments.Count -ne 0) { Block-FormalPackageLauncher }
foreach ($name in @(
  "NODE_OPTIONS",
  "NODE_PATH",
  ...
  if ([Environment]::GetEnvironmentVariable($name, "Process") -ne $null) {
    Block-FormalPackageLauncher
  }
}

$launcherPath = (Resolve-Path -LiteralPath $PSCommandPath).Path
$runnerPath = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "run-formal-windows-package-qa.mjs")).Path
...
if ((& $nodePath --version) -notmatch '^v22\.') { Block-FormalPackageLauncher }
