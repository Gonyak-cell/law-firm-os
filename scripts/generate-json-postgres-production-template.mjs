...
if (!option("--output") || !outsideWorktree(output)) throw new Error("--output must be outside the worktree");
if (existsSync(output) || (existsSync(dirname(output)) && lstatSync(dirname(output)).isSymbolicLink())) {
  throw new Error("production template output must be a new non-symlink path");
}
...
const validation = validateJsonPostgresProductionTemplate(template);
