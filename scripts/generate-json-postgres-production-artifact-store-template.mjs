...
if (!requested || !outsideWorktree(output)) {
  throw new Error("--output must be outside the worktree");
}
if (existsSync(output)
  || (existsSync(dirname(output)) && lstatSync(dirname(output)).isSymbolicLink())) {
  throw new Error("artifact-store template output must be a new non-symlink path");
}
const template = buildJsonPostgresProductionArtifactStoreTemplate();
const validation = validateJsonPostgresProductionArtifactStoreTemplate(template);
