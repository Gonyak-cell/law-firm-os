...
const PACKAGED_RENDERER_URL = "matter-app://app/index.html?desktop=1";
const originOptions = {};
...
test("origin policy allows only approved dev and packaged renderer origins", () => {
  ...
  assert.equal(PACKAGED_RENDERER_ORIGIN, "matter-app://app");
  ...
  assert.equal(isApprovedRendererUrl(PACKAGED_RENDERER_URL), true);
  assert.equal(isApprovedRendererUrl("file:///tmp/index.html", originOptions), false);
  assert.equal(isApprovedRendererUrl("file:///Applications/matter.app/Contents/Resources/app/src/renderer/web/index.html", originOptions), false);
  assert.equal(isApprovedRendererUrl("matter-app://evil/index.html", originOptions), false);
...
test("navigation guards deny unapproved navigations and window opens", () => {
  ...
  assert.deepEqual(windowOpenHandler({ url: APPROVED_DEV_RENDERER_URL }), { action: "deny" });
  assert.deepEqual(windowOpenHandler({ url: PACKAGED_RENDERER_URL }), { action: "deny" });
