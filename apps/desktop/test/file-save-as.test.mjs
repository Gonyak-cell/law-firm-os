...
function saveAsHarness({ allowed = true, canceled = false } = {}) {
  const order = [];
  const writes = [];
  const auditEvents = [];
  const dialog = {
  ...
  return { auditEvents, controller, dialog, order, writes };
}

test("save-document-as uses save dialog before writing user-selected path", async () => {
  ...
  assert.deepEqual(harness.order, ["precheck", "dialog", "writer"]);
