...
test("OUTM31 all-functions precedent browser contract is authoritative and session-fenced", async () => {
  ...
  try {
    ...
    await page.locator("[data-testid='outlook-precedent-search-submit']").click(); await search;
    ...
    await page.locator("[data-testid='outlook-precedent-next']").click(); await next;
    ...
    state.appendTransient = true; state.appendAttempts = 0; const transientNext = page.waitForResponse((r) => r.status() === 200 && new URL(r.url()).pathname === "/api/outlook/precedents" && new URL(r.url()).searchParams.get("cursor") === "cursor-1"); await page.locator("[data-testid='outlook-precedent-next']").click(); await transientNext;
    ...
    state.currentLeak = true; const leaked = page.waitForResponse((r) => new URL(r.url()).pathname === "/api/outlook/precedents" && !new URL(r.url()).searchParams.has("cursor")); await page.locator("[data-testid='outlook-precedent-search-submit']").click(); await leaked;
