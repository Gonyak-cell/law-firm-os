...
test("office-only persisted token survives an early office-ready restart", async () => {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/auth/session") {
      ...
      return route.fulfill(json({ authenticated: true, principal: { user_id: "office-only-user", tenant_id: "office-only-tenant" } }));
    }
    if (url.pathname === "/api/outlook/connection") {
      return route.fulfill(json({ item: { status: "connected", active: true, connection_id: "m365_connection_office_storage_qa", state_version: 1, mailbox_address: "qa@example.invalid" } }));
    }
  ...
  try {
    ...
    const secondSessionRequest = page.waitForRequest((request) => request.url().endsWith("/api/auth/session"));
    ...
    assert.equal(await page.evaluate(() => window.__OFFICE_ONLY_GET_CALLS >= 2), true);
...
test("token installation owns B before a held Office write and never sends an authless restart read", async () => {
  ...
  await page.addInitScript((token) => {
    ...
    const mailbox = {
      ...
      userProfile: { emailAddress: "qa@example.invalid" }, addHandlerAsync() {}, removeHandlerAsync() {}, convertToRestId(id) { return id.replace("office-", "rest-"); },
    };
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/auth/session") { sessionRequests.push(route.request()); return route.fulfill(json({ authenticated: true, principal: { user_id: "token-race-user", tenant_id: "token-race-tenant" } })); }
    if (url.pathname === "/api/outlook/connection") return route.fulfill(json({ item: { status: "connected", active: true, connection_id: "m365_connection_office_storage_qa", state_version: 1, mailbox_address: "qa@example.invalid" } }));
