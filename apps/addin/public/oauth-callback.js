...
const message = callbackStatus
  ? { type: "lawos-outlook-oauth", status: callbackStatus }
  : {
...
function send({ finalAttempt = false } = {}) {
  ...
  const statusOnly = ["connected", "failed"].includes(message.status);
  if (!statusOnly && (!message.state || (!message.code && !message.error))) {
    delivered = true;
  ...
  try {
    ...
    delivered = true;
    show(message.status === "connected"
      ? "연결 완료를 확인했습니다. 이 창을 닫아도 됩니다."
      : "연결 응답을 전달했습니다. 이 창을 닫아도 됩니다.");
    return true;
  } catch {
