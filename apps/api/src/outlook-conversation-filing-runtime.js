...
import { outlookSourceItemKey } from "../../../packages/email-dms/src/outlook-source-identity.js";
...
function resolvedMessage(policy, canonical, clock, filingMode) {
  ...
  const sourceIdentity = {
    canonical_graph_message_id: canonical.immutable_message_id,
    rest_message_id: canonical.rest_message_id,
  ...
  const thread = Object.freeze({
    ...
    ...sourceIdentity,
    item_key: outlookSourceItemKey(sourceIdentity),
    subject: canonical.subject,
    from: canonical.from ?? {},
    to: recipients("to"),
    cc: recipients("cc"),
...
export function createOutlookConversationFilingRuntime({
  ...
  return Object.freeze({
    async fileCanonicalMessage({ policy, canonical, connection, actor_id, authorized_by_actor_id } = {}) {
      ...
      const filingMode = canonical.folder_kind === "sentitems" ? "sent" : "manual";
      const resolvedCanonical = resolvedMessage(policy, canonical, clock, filingMode);
      const filingEmail = { ...resolvedCanonical.thread };
      delete filingEmail.graph_message_id;
      return request_runtime_authority.run({
        ...
        command: async (runtimes) => {
          ...
          const result = await file_resolved({
            body: { tenant_id: policy.tenant_id, matter_id: policy.matter_id, email: filingEmail },
            context,
