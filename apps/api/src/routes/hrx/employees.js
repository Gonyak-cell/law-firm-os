...
import { publicEmployeeDisplayName } from "../../../../../packages/hrx/src/people-presentation.js";
...
function mapError(error) {
  ...
  if (/already exists/i.test(String(error?.message ?? ""))) {
    return response(409, {
      outcome: "blocked",
      safe_error_code: "HRX_EMPLOYEE_ID_ALREADY_EXISTS",
      reason: "Employee identifier already exists",
    });
  }
  const status = Number.isInteger(error?.status) ? error.status : 400;
  return response(status, {
    outcome: "blocked",
    safe_error_code: error?.safe_error_code ?? "HRX_EMPLOYEE_ROUTE_ERROR",
    ...
    async handle(request = {}) {
      try {
        if (request.method === "POST") {
          ...
          return response(201, { outcome: "created", employee: publicEmployee(employee) });
        }
        if (request.method === "GET") {
          ...
          return response(200, { outcome: "ok", employee: publicEmployee(employee) });
        }
        if (request.method === "PATCH") {
          const employee = await service.updateEmployee(request.context, { employee_id: request.params?.employee_id }, request.body);
          return response(200, { outcome: "updated", employee: publicEmployee(employee) });
        }
