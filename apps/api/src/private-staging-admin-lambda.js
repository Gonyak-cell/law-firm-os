...
export const PRIVATE_STAGING_BOOTSTRAP_APPROVAL_ID = "lawos-private-staging-synthetic-migration-20260720";
export const PRIVATE_STAGING_CUT005_ACTION = "lawos-private-staging-cut-005";
export const PRIVATE_STAGING_CUT005_APPROVAL_ID = "lawos-private-staging-cut-005-20260720";
...
function normalizeManifest(input) {
  if (input?.schema_version !== "law-firm-os.synthetic-staging-manifest.v1") throw new TypeError("synthetic manifest schema is invalid");
  ...
  if (tenantIds.length < 2) throw new TypeError("at least two synthetic tenants are required");
  return Object.freeze({ tenant_ids: Object.freeze(tenantIds), manifest_sha256: sha256(JSON.stringify(input)) });
}
...
export async function bootstrapPrivateStagingDatabase({
  ...
  assertDirectInvoke(event, { action: PRIVATE_STAGING_BOOTSTRAP_ACTION, approvalId: PRIVATE_STAGING_BOOTSTRAP_APPROVAL_ID });
  const {
...
export async function executePrivateStagingCut005({
  ...
  assertDirectInvoke(event, { action: PRIVATE_STAGING_CUT005_ACTION, approvalId: PRIVATE_STAGING_CUT005_APPROVAL_ID });
  ...
  try {
    ...
    return Object.freeze({
      ...
      owner_instruction_sha256: instructionSha,
      synthetic_manifest_sha256: manifest.manifest_sha256,
      approval_id: PRIVATE_STAGING_CUT005_APPROVAL_ID,
      secret_material_returned: false,
      production_ready_claim: false,
    });
  } finally {
...
export async function handler(event = {}) {
  try {
    ...
    return Object.freeze({
      outcome: "BLOCKED",
      action: [PRIVATE_STAGING_BOOTSTRAP_ACTION, PRIVATE_STAGING_CUT005_ACTION].includes(event.action)
        ? event.action
