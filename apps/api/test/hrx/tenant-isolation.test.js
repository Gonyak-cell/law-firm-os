...
async function json(path, options = {}) {
  const headers = path.startsWith("/api/hrx") ? { ...tenantBHeaders, ...(options.headers ?? {}) } : options.headers;
...
test("tenant isolation keeps trusted tenant-b from seeing tenant-a fixture rows", async () => {
  ...
  assert.equal(analytics.body.analytics.tenant_id, "tenant_b_qa_synthetic");
