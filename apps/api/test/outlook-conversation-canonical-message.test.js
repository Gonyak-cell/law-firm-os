...
test("OUTM-28 canonical source preserves recipients and exact Inbox provenance", async () => {
  ...
  const canonical = await source.getOwnMessage({
    ...
    message_id: "rest-message-outm28",
    resource: "me/mailFolders('inbox')/messages",
  });
  assert.equal(calls[0].rest_message_id, "rest-message-outm28");
  assert.equal(canonical.rest_message_id, "rest-message-outm28");
  assert.equal(canonical.immutable_message_id, "message-outm28");
