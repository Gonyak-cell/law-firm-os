...
async function authorityFixture(requestId, kind, digest, overrides = {}, filePath = undefined) {
  ...
  if (filePath) await authorityRepository.replaceState(authorityRepository.loadState());
...
test("OUTM-34 DMS completion boundary writes an idempotent immutable PDF version with matching SHA", async (t) => {
  ...
  const authority = await authorityFixture("request-api", "signed_pdf", digest);
...
test("OUTM-34 authority drift after approved read is rejected before local DMS storage", async (t) => {
  ...
  const digest = createHash("sha256").update(bytes).digest("hex");
  const authority = await authorityFixture("request-fenced", "signed_pdf", digest);
  let entered;
  ...
  const approvedDocumentResolver = async () => {
    if (firstResolver) {
      ...
      return {
        ...authority.source,
        authority: { ...authority.source.authority, permission_envelope_id: "permission-drift", audit_trace_id: "audit-drift" },
        document: { ...authority.source.document, permission_envelope_id: "permission-drift", audit_trace_id: "audit-drift" },
      };
    }
  ...
  release();
  await assert.rejects(ingest, (error) => error?.safe_error_code === "DOCUSIGN_APPROVED_SOURCE_MISMATCH");
...
test("OUTM-34 shared file authority lock spans async callback and releases after rejection", async (t) => {
  ...
  const digest = createHash("sha256").update(bytes).digest("hex");
  const authority = await authorityFixture("request-lock", "signed_pdf", digest, {}, authorityPath);
  ...
  const queuedReplacement = peer.replaceState(peer.loadState()).then(() => { replacementSettled = true; });
  await Promise.resolve();
  assert.deepEqual([peerEntered, replacementSettled], [false, false]);
  release();
  assert.deepEqual(await Promise.all([first, second, queuedReplacement]), ["first", "second", undefined]);
...
test("OUTM-34 shared file barrier inside storage.putObject fails closed with zero DMS writes", async (t) => {
  ...
  const digest = createHash("sha256").update(bytes).digest("hex");
  const authority = await authorityFixture("request-storage-barrier", "signed_pdf", digest, {}, authorityPath);
  symlinkSync(authorityPath, authorityAlias);
  const peer = createDocusignEnvelopeRepository({ filePath: authorityAlias });
