...
import { filingAuditMetadata } from "../../../packages/email-dms/src/email-filing-canonical.js";
...
const GRAPH_ID = "immutable:pg-receipt-authority";
const INTERNET_ID = "<pg-receipt-authority@amic.law>";
const CONVERSATION_ID = "conversation-pg-receipt-authority";
const THREAD_ID = "thread:pg-receipt-authority";
const SUBJECT = "PostgreSQL receipt authority";

function pgCounts(adminPool) {
...
test("PostgreSQL DMS authority binds Outlook replay to live rows and HTTP readback", async (t) => {
  ...
  await assert.rejects(fileEmailThreadToMatter({ repository, thread, actor_id: ACTOR, require_original_mime_document: true, idempotency_key: key, durable_mime_authority: uploadRuntime }), /authority/u);
  ...
  await assert.rejects(fileEmailThreadToMatter({ repository, thread, actor_id: ACTOR, require_original_mime_document: true, idempotency_key: key, durable_mime_authority: uploadRuntime, durable_mime_document: staleState }), /snapshot/u);
