...
function fixture({ audit = "match" } = {}) {
  ...
  if (audit !== "missing") {
    repository.appendAudit({ event_id: `outlook.email.file:${TENANT}:${emailThreadId}`, tenant_id: TENANT, actor_id: audit === "foreign" ? "foreign-actor" : "original-actor", action: "dms.email.thread.file", object_type: "DmsEmailThread", object_id: emailThreadId, decision: "allow", reason: "email_thread_filed_to_matter", occurred_at: audit === "stale" ? "1999-01-01T00:00:00.000Z" : thread.filing_time, metadata: audit === "foreign" ? { tenant_id: TENANT, matter_id: "matter:foreign", email_thread_id: emailThreadId, graph_message_id: thread.graph_message_id, internet_message_id: thread.internet_message_id, conversation_id: thread.conversation_id, filing_mode: "manual", filed_document_ids: [documentId], actor_id: "foreign-actor" } : { tenant_id: TENANT, matter_id: MATTER, email_thread_id: emailThreadId, graph_message_id: thread.graph_message_id, internet_message_id: thread.internet_message_id, conversation_id: thread.conversation_id, filing_mode: "manual", filed_document_ids: [documentId], actor_id: "original-actor" } });
  }
  repository.recordIdempotency({ tenant_id: TENANT, idempotency_key: key, operation: "outlook_email_file", request_fingerprint: null, response: { outcome: "created", email_thread_id: emailThreadId, matter_id: MATTER, filed_document_ids: [documentId] } });
