...
export async function docusignRuntime({ authorizeMatter = async () => ({ allowed: true, authority_binding: AUTHORITY_BINDING }), prepare = true, webhookRequestResolver } = {}) {
  ...
  const eventService = createDocusignEnvelopeEventService({ repository, connectionResolver, webhookRequestResolver, resolveSecret: async ({ ref }) => ref === CONNECTION.hmac_secret_ref ? HMAC_SECRET : null, adapter, receiptStore: { async put(input) { return { receipt_ref: `receipt:${input.sha256}`, sha256: input.sha256, immutable: true }; } }, artifactStore: { async ingest(input) { return { document_id: `dms:${input.kind}`, version_id: `version:${input.kind}`, sha256: input.sha256, ...input, immutable: true }; } }, clock: () => "2026-08-08T02:05:00.000Z" });
