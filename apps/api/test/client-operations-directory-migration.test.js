...
import { createOutlookAuthorityPostgresFixture } from "./support/outlook-authority-postgres-fixture.js";

const DIRECTORY = "149_hrx_049_hrx_directory_authority";
const FULL_CATALOG = listClientOperationsPostgresMigrations();
const ASSIGNMENT_INDEX = FULL_CATALOG.findIndex(({ id }) =>
  id === "306_client_outlook_desktop_assignment");
assert.ok(ASSIGNMENT_INDEX > 0);
const CATALOG = FULL_CATALOG.slice(0, ASSIGNMENT_INDEX);
...
function runDirectoryPrefixThrough305(pool, { appliedBy } = {}) {
  return runPostgresMigrations(pool, {
    migrations: CATALOG,
    appliedBy,
    allowedHistoricalGapIds: [DIRECTORY],
  });
}
...
test("pre-007 client history missing only HRX directory runs only 149", async () => {
  const calls = [];
  const result = await runDirectoryPrefixThrough305(historyPool(PRE_DIRECTORY, calls));
...
test("directory gap does not permit unrelated history or checksum drift", async (t) => {
  ...
  for (const [name, rows, code = "LAWOS_POSTGRES_MIGRATION_HISTORY_DIVERGED"] of scenarios) {
    await t.test(name, async () => {
      const calls = [];
      await assert.rejects(runDirectoryPrefixThrough305(historyPool(rows, calls)), { code });
...
test("real PostgreSQL pre-007 directory upgrade preserves history and rows, rolls back failures, and replays zero", async (t) => {
  const fixture = await createOutlookAuthorityPostgresFixture(t);
  if (!fixture) return;
  await runDirectoryPrefixThrough305(fixture.adminPool);
  ...
  await assert.rejects(runDirectoryPrefixThrough305(failingPool));
  assert.deepEqual(await readHistory(), beforeHistory);
  assert.deepEqual(await readColumns(), []);
  assert.deepEqual(await readRows(), beforeRows);

  const upgraded = await runDirectoryPrefixThrough305(adminPool, { appliedBy: "synthetic-directory-upgrade" });
  ...
  assert.equal((await verifyPostgresMigrationState(adminPool, { migrations: CATALOG })).length, CATALOG.length);

  const replay = await runDirectoryPrefixThrough305(adminPool, { appliedBy: "synthetic-directory-replay" });
