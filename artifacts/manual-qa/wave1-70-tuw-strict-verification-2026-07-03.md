...
- External receipt readiness addendum: `node scripts/validate-upl-c09-outlook-external-receipt.mjs`, `node scripts/run-wave1-external-receipt-readiness.mjs`, `node scripts/validate-wave1-external-receipt-readiness.mjs`, and `node --test scripts/test/wave1-external-receipt-readiness.test.mjs` PASS. This readiness ledger documents the exact C09/B13 external receipts still missing, keeps inherited C10-C12/E04 non-PASS, creates a sanitized C09 Outlook receipt intake contract, and makes no 70/70 completion claim.
- Remediation closeout validator addendum: `node scripts/run-wave1-remediation-strict-verification-proof.mjs`, `node scripts/validate-wave1-remediation-strict-verification.mjs`, and `node --test scripts/test/wave1-remediation-strict-verification.test.mjs` PASS. This writes `artifacts/manual-qa/wave1-remediation-strict-verification-2026-07-03.json`, validates the remediation workbook/addendum against the preserved 70-row matrix, and fails if C09/B13 external blockers are promoted without receipts.
...
| UPL-B-13 | PARTIAL | Local 3.3% Korean business-income withholding model and TaxInvoice proof now pass: `calculateKoreanBusinessIncomeWithholding` computes 3% income tax plus 0.3% local income tax, carries withholding/net payable on `TaxInvoice`, and proof `artifacts/manual-qa/upl-b13-withholding-proof.json` verifies KRW 1,000,000 -> KRW 33,000 withholding -> KRW 967,000 net. Popbill sandbox readiness stores prepared request/mgt-key hashes and validates hash-only provider probe/body/error handling. `artifacts/manual-qa/wave1-external-receipt-readiness-2026-07-03.json` keeps the strict boundary explicit: PASS still requires external tax-invoice sandbox roundtrip; it is not claimed. |
...
| UPL-C-09 | BLOCKED | Local taskpane proof exists, `scripts/validate-upl-c09-outlook-external-receipt.mjs` now validates the sanitized operator receipt contract, and `artifacts/manual-qa/wave1-external-receipt-readiness-2026-07-03.json` links it to the inherited Outlook rows. Current state remains Outlook web/new desktop smoke false, Entra consent false, provider runtime false, and external receipt present false. |
