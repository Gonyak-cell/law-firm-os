...
- 문서 상태: 브랜치 구현·로컬 집중 검증 완료 — P0 기준 계약 5/5, P1 은행 입금 매출 7/7, P2 수임료·미수금 6/6, P3 Outlook 문의·상담·수임 12/12, P4 Client 운영 조회 5/5, P5 Client 10개 메뉴 11/11, P6 신뢰성·이전·출시 도구 7/7, 전체 로컬 구현 53/53 완료. 단, exact-main·운영 배포·실제 Graph/AWS·go-live 증거는 승인 후 별도 출시 게이트이며 현재 `BLOCKED_EXTERNAL`이다.
...
이 계획은 이전 33개 초안을 대체한다. v2 ID를 최초 실행 원장으로 사용하며, 2026-07-31 현재 P0의 5개 TUW, P1의 7개 TUW, P2의 6개 TUW, P3의 M365 연결·원본 메일 증거·문의 등록·Outlook 추가 기능·문의 조회·상담·수임 결정·Intake 인계 12개 TUW, P4의 고객 권한 선필터·5개 KPI·오늘 확인할 일·12개월 추이·고객 순위·통합 대시보드 API 5개 TUW, P5의 Client 메뉴·직접 주소, 대시보드 KPI·오늘 확인할 일, 월별 입금 매출·문의 현황·고객 순위, 고객 목록·상세, 신규 고객 등록, 새 문의 목록·상세, 수임 현황, 상담·수임 관리·접촉 이력, 입금 매출 내역, 수임료·미수금, 고정 리포트 11개 TUW와 P6의 보안·권한, PostgreSQL 이전·복구, 기존 자료 대사, 180상태 화면 검증, 장애 대응 절차, 출시 검증 도구 7개 TUW를 브랜치에서 구현·로컬 집중 검증했다. 실제 Entra·시험 mailbox·Graph/AWS·운영 배포·로그인 패키지 증거는 구현 완료와 구분해 별도 승인 게이트로 남긴다.
...
| 수임하지 않음 | `engagement_decision=declined`, Opportunity `closed_lost` 또는 Lead `inquiry_status=closed` | 종료 사유 확인, 권한 있는 재개 |

기존 Opportunity 단계 `new → qualified → intake_requested → intake_opened → closed_won/closed_lost`는 유지한다.

여러 사실이 동시에 남아 있으면 `수임 확정 → 수임하지 않음 → 수임 검토 중 → 상담 예정 → 새 문의 → 확인 중` 순으로 한 상태만 표시하고, 충돌한 사실은 별도 확인 대상으로 표시한다.
...
| 신규 고객 중복 확인·등록 | `master_data.client.write` + 중복 후보별 `analytics.client.read` | 운영 관리자, 파트너, LawOS 관리자 |
...
이 원장의 `완료`는 해당 TUW의 브랜치 구현과 로컬·합성 검증이 끝났다는 뜻이다. 실제 Entra/시험 mailbox, AWS, 배포, 운영 데이터, go-live 증거까지 얻었다는 뜻이 아니며, 이 외부 증거는 본문에 적힌 출시 차단 조건과 별도 승인 경계를 그대로 따른다. 대시보드가 만든 상세 이동 주소처럼 후속 메뉴가 소비해야 하는 계약도 생산자 TUW의 완료와 별개로 후속 TUW와 P6 최종 E2E에서 다시 검증하며, 소비자가 끝나기 전에는 전체 Goal을 완료로 판정하지 않는다.
...
| `CL-P0-W01-T04` | 완료 | 문의·수임·계약·Matter 분리와 10개 Client 기능 권한, 5개 사용자 유형 매핑 등록 | 서명 세션 허용·차단 및 production 최고권한 복원 테스트 |
...
| `CL-P3-W02-T04` | 완료 | `POST /api/crm/consultations/:id/outlook-event`를 별도 명령으로 등록해 서명된 tenant·상담 권한·기대 version·사유·요청키를 Graph 호출 전에 확인하고, 상담이 진행 중이며 Outlook 일정이 아직 없을 때만 사용자가 누른 요청을 실행; tenant와 상담 ID로 항상 같은 UUID형 `transactionId`를 만들고 앱의 UTC 시작·종료 시각만 `/me/events`에 보내며 제목은 고객명·상담 내용이 없는 `법률 상담`, 공개 범위는 `private`, 일정 상태는 `busy`로 고정하고 본문·참석자·공유 메일함 필드는 받거나 보내지 않음; Graph가 돌려준 event ID·허용된 Outlook HTTPS webLink·transaction ID·생성자·시각·일정 해시를 Activity version과 함께 보존하고 provider request ID는 해시만 저장; 일반 상담 응답에는 webLink와 연결 상태만 보여 주고 event ID·transaction ID는 별도 필드로 내보내지 않으며 감사에도 원문 식별값·상담 내용을 남기지 않음; 같은 버튼을 다시 누르면 저장된 결과를 반환해 provider를 다시 부르지 않고, Graph 성공 뒤 로컬 저장이 실패한 경우에도 같은 transaction ID로 재실행해 외부 event 한 건에 로컬 기록을 복구; 이후 앱에서 시각이 달라지면 자동으로 Outlook을 고치지 않고 `update_required`로 구분; 기능 스위치·provider runtime·연결·`Calendars.ReadWrite`가 준비되지 않으면 제품 상태를 바꾸지 않고 차단 | `VC-CL-CON-002` 서비스·포트·실제 Graph 요청 형식·API 재클릭·권한·stale version·입력 위조·기능 꺼짐·저장 실패 복구 집중 21/21, CRM·Email DMS·Outlook·Client API 결합 회귀 223/223, 실제 임시 PostgreSQL API authority 8/8, 32개 Client 시나리오 fixture와 권한 계약 40/40, RP09 계약 validator PASS; Graph `/me/events` 실제 호출·관리자 동의·시험 일정 생성 후 삭제 영수증은 없으므로 출시는 계속 차단하며 AWS·배포·운영 데이터 변경 없음 |
| `CL-P3-W03-T01` | 완료 | `POST /api/crm/inquiries/:id/engagement-decisions`에서 변호사·파트너·관리자의 명시적 결정을 받고 `수임 확정`·`수임하지 않음`·`검토 중`을 계약서 서명 상태와 분리해 Opportunity version과 함께 기록; 수임 확정은 먼저 CRM 결정과 단계 처리 기록을 원자적으로 남긴 뒤 문의의 Party를 유일한 기존 ClientGroup에 연결하거나 결정적 ID의 새 그룹으로 등록하고, 원화 정수 금액 또는 사용자가 확인한 `금액 미입력`의 FeeCommitment를 생성; Matter ID·자동 Matter 생성·`closed_won` 전환은 받거나 수행하지 않으며 Opportunity는 현재 단계에 유지; 각 단계는 파생 요청키의 해시와 완료 영수증을 보존해 재실행에서 끝난 단계를 건너뛰고, 고객·금액 반영 중 실패하면 원문 오류 없이 `반영 확인 필요`로 표시하며 권한을 다시 확인하는 `/engagement-repair`가 실패한 단계만 이어서 처리; 결정 저장 또는 재처리 시작 직후 프로세스가 중단되어도 같은 요청 영수증으로 `처리 중`인 남은 단계만 재개하고, 반영 확인이 필요한 동안에는 다른 결정을 넣지 못하게 차단; 수임하지 않음은 종료 사유를 의무화하고 `closed_lost`로 닫으며, 수임 확정을 검토 중으로 되돌리면 기존 FeeCommitment를 취소하고 활성 배분을 해제하고, 수임하지 않음을 되돌리면 문의와 검토 단계를 다시 엶; 공통 문의 조회에는 결정 version, 반영 상태, 고객그룹·수임료 참조와 자연스러운 상태 문구만 제공하고 단계 요청키·원문 오류·종료 사유는 일반 응답에서 제외; 파일 저장소와 PostgreSQL 요청 권위가 CRM·Master Data·Finance의 같은 결과를 보존하도록 서버의 기본 Master Data 저장소를 공용 권위로 통일 | `VC-CL-ENG-001`~`004`의 금액 입력·금액 미입력·부분 실패 후 재처리·거절 사유·재검토·결정/재처리 중단 복구 집중 service/API 9/9, CRM 전체 107/107, Billing 전체 144/144, 실제 서명 세션·상담·문의·Outlook·Master Data·production 권한·PostgreSQL 요청 권위 결합 46/46, 실제 임시 PostgreSQL의 세 도메인 레코드와 단계 영수증 9/9, 32개 Client 시나리오 fixture와 권한 계약 40/40, RP09·G3-A validator와 `git diff --check` PASS; Matter 생성 0건, 일반 직원 선차단, 다른 tenant·stale version·같은 키의 다른 명령·Matter 직접 지정은 상태 변경 전에 거절; sloplint는 신규 코드·문구 신호 없이 기존 OSS 출처 URL 4건만 약하게 오탐; AWS·Graph·배포·운영 데이터 변경 없음 |
...
| `CL-P4-W01-T04` | 완료 | `ClientOperationsReadModel.readTrendsAndRankings`가 선계산한 고객 범위와 기존 CRM 문의 projection·입금 매출·미수금 서비스를 재사용해 기준일을 포함한 최근 12개월 월별 입금 매출, 여섯 문의 상태, 고객별 매출·미수금 순위를 계산; 월별 그래프는 11개월 전 첫날부터 Asia/Seoul 기준일까지 정확히 12개 월을 0원 달도 포함해 반환하고, 매출 순위는 `이번 달`·`이번 분기`·`올해 누적`을 선택하되 기본값을 올해 누적으로 고정하며 기준일 이후 거래는 제외; 문의 현황은 `새 문의`·`확인 중`·`상담 예정`·`수임 검토 중`·`수임 확정`·`수임하지 않음`의 공통 상태 우선순위와 표시 순서를 그대로 사용하고 합계가 접근 가능한 projection 수와 다르면 fail-closed; 매출 순위는 순입금 매출→최근 입금일→고객명→고객 ID, 미수금 순위는 미수금→가장 이른 납부기한→고객명→고객 ID라는 기존 서비스 정렬을 보존하며 모든 월·상태·고객 순위에 hash가 아닌 화면 의미 기반 상세 이동 정보를 제공; 대시보드용 응답에는 은행 원본·거래 상세를 넣지 않고 차단 고객의 이름·ID·손상 거래·금액 미입력 약정을 원천 정규화 전에 제외 | `VC-CL-DASH-001` 기준 fixture와 기간 선택·동률·기준일 이후·차단 고객·잘못된 기간 선차단 집중 검증 3/3, Client 읽기 모델 누적 13/13, Analytics 전체 116/116, 기존 매출·미수금 24/24, 문의 공통 projection·실제 API 6/6, 실제 임시 PostgreSQL 요청 권위에서 수임 확정 1건·미수금 1,200만 원·12개월 0원 추이 1/1, 32개 Client 시나리오 fixture와 역할 계약 40/40 및 RP15 Analytics 계약 validator PASS, `git diff --check`·sloplint PASS; 기준 fixture는 2025-08~2026-07 12개월, 문의 상태 1·0·1·1·1·1건, 올해 누적 매출 새봄테크 2,500만 원·한빛건설 1,100만 원, 미수금 한빛건설 900만 원으로 일치; 공개 대시보드 API·AWS·배포·운영 데이터 변경 없음 |
| `CL-P4-W01-T05` | 완료 | `GET /api/analytics/clients/dashboard`가 선계산한 고객 권한 범위 안에서 KPI·오늘 확인할 일·최근 12개월 입금 매출·문의 현황·기간별 매출 순위·미수금 순위를 한 응답으로 묶고, 고객 정보·CRM·입금 매출·미수금·입금 연결 확인·금액 미입력 수임료마다 확인 시각·최종 레코드 시각·안전한 건수·`available/no_data/permission_denied/error` 상태를 반환; 한 원천 장애나 권한 부재는 다른 원천 값을 보존한 `partial`로 격리하고 누락 값을 0으로 가장하지 않으며, 고객 없음·운영 데이터 없음·고객 권한 없음·고객 원천 장애는 각각 `empty/no_data`, `empty/no_data`, 403, 503으로 구분; 일반 직원의 은행 분류 권한 부재는 입금 연결 업무만 가리고 나머지 KPI·그래프·순위를 유지하며, 원천 payload·은행 원문·인증정보·권한 밖 고객 건수와 금액은 응답에 넣지 않음 | 새 대시보드 읽기 모델 누적 17/17과 실제 서명 세션 HTTP 5/5, Analytics 전체 120/120, 기존 매출·미수금 24/24, 문의 공통 projection·실제 API 6/6, 신규·기존 Analytics API 결합 18/18, 실제 임시 PostgreSQL 수임 확정·미수금·묶음 응답 1/1, 32개 시나리오 fixture와 역할 계약 40/40 및 RP15 validator PASS; CRM 장애 때 문의 KPI `null`·Finance 값 유지, 고객 원천의 내부 오류 문구 비노출, `git diff --check`·sloplint PASS; AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W01-T01` | 완료 | Client 사이드바를 `대시보드`, `고객 목록`, `신규 고객`, `새 문의`, `입금 매출 내역`, `수임 현황`, `상담·수임 관리`, `접촉 이력`, `수임료·미수금`, `리포트`의 10개 메뉴로 고정하고 화면 제목도 같은 자연스러운 한국어로 맞춤; 기존 계정·연락처·관계 주소는 고객 목록으로, Intake·이해상충·계약 주소는 상담·수임 관리로 합치며 데이터 관리·가져오기·Client 설정은 대시보드로 차단 안내 후 이동하고 알 수 없는 주소는 별도 찾을 수 없음 안내를 제공; 화면 허용 목록에서도 숨은 9개 section을 제외하고 같은 문서 안에서 해시만 바뀌는 직접 이동까지 정식 주소로 교체 | `VC-CL-ROUTE-001` 계약 1/1, UI 회귀 32/32, Home·Client·Matter·People 실제 브라우저 회귀 27/27, Client 병합·비활성·없는 직접 주소와 720px 가로 넘침 확인, web typecheck·production build·`git diff --check` PASS, sloplint 신규 경고 없음; AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W01-T02` | 완료 | Client 대시보드가 통합 `GET /api/analytics/clients/dashboard` 한 번으로 `새 문의`, `오늘 상담`, `수임 검토 중`, `이번 달 입금 매출`, `총 미수금` 5개 KPI와 서버 우선순위의 `오늘 확인할 일`을 표시하도록 연결; 각 지표·업무는 실제 Client 메뉴와 필터로 이동하고 대상 `record_id`·`inquiry_id`를 후속 목록·상세 화면이 사용할 주소 계약으로 전달; 전체 로딩·데이터 없음·권한 없음·확인 필요·오류와 원천별 일부 실패를 구분하고 CRM 장애 때 문의·상담·수임 건수를 0으로 바꾸지 않은 채 Finance 금액을 유지하며, 응답 형식이 잘못되거나 알 수 없는 이동 대상은 안전하게 차단; 내부 ID·담당자 ID·원천 오류 원문은 화면에 표시하지 않음 | 기준 fixture의 KPI 1건·1건·1건·3,300만 원·900만 원과 오늘 확인할 일 5건, KPI·업무별 메뉴/필터/대상 주소 집중 4/4; 권한 없음·확인 필요·데이터 없음·오류와 CRM 일부 실패 실제 브라우저 3/3, UI 회귀 32/32, Home·Matter·Client 전체 브라우저 회귀 28/28, web typecheck·production build·`git diff --check` PASS; 1440px·390px 실제 화면에서 KPI 한 줄 배치·세로 재배치와 가로 넘침 없음 확인; 대상 레코드 자동 선택은 후속 목록·상세 TUW가 주소 계약을 소비할 때 완성하며 AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W01-T03` | 완료 | 같은 통합 대시보드 응답의 최근 12개월 순입금 매출을 0원 월과 환불로 음수가 된 월까지 보존하는 접근 가능한 막대그래프로 표시하고, `새 문의`·`확인 중`·`상담 예정`·`수임 검토 중`·`수임 확정`·`수임하지 않음` 여섯 문의 상태를 전체 합계와 함께 고정 순서로 표시; 고객 매출 순위는 기본 `올해 누적`, 미수금 순위는 현재 잔액 기준으로 각각 상위 10개를 고객명·금액·최근 입금일 또는 납부기한과 함께 보여 주며 전체 합계와 금액 미입력 건수를 별도로 유지; 월별 합계·문의 합계·순위 합계 또는 순위 순서가 서버 응답과 맞지 않으면 해당 구역만 오류로 차단하고 CRM 장애 때 문의 현황만 실패 상태로 두면서 월별 매출·두 순위를 유지; 그래프의 월, 문의 상태, 고객 순위 선택 시 실제 Client 메뉴와 `month`·`filter`·`record_id`·`tab`·`period` 주소 계약으로 이동하며 내부 고객 ID는 화면에 표시하지 않음 | 모델·합계·상위 10개·음수 월·잘못된 응답 집중 8/8, 통합 UI 회귀 32/32, Home·Matter·Client 전체 실제 브라우저 회귀 29/29, CRM 일부 실패와 Finance 구역 유지 확인; 1440·820·390px 자동 배치·가로 넘침·사이드바 전환 완료 상태 3/3 및 실제 캡처 수동 점검, web typecheck·production build·`git diff --check` PASS; 신규 차트·문구 AI slop 신호 없음, 기존 공통 CSS와 OSS 주소의 약한 경고만 유지; AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W02-T01` | 완료 | `GET /api/analytics/clients`가 활성 ClientGroup에 `analytics:client:read` 규칙과 고객별 객체 ACL을 먼저 적용한 뒤 고객명·상태·고객 유형·정확한 구성원 수·대표 정보 유무만 반환하고, 누락 고객 수와 내부 Party·Entity ID는 버림; `GET /api/analytics/clients/:client_group_id/operations`는 같은 허용 고객에 한해 개요·연락처·Matter·문의 자료를 독립 원천 상태와 함께 반환하며, 연락처는 `primary_contact` 등 명시적인 연락처 관계만 사용해 상대방·동명이인을 제외하고 연락처 원문은 항상 보호함; Matter·문의·상담은 명시 ID 연결과 원천별 읽기 권한·객체 ACL을 적용하고 일부 객체가 빠지면 허용 자료만 유지한 `partial`로 표시하면서 누락 개수는 `null`로 고정함; 권한 밖 고객과 없는 고객은 후속 원천을 읽지 않은 동일 404로 처리하고, 웹은 허용 목록에 있는 `record_id`만 상세 API에 보내며 `overview`·`contacts`·`matters`·`inquiries` 탭 및 대시보드의 `deposit_revenue`·`receivables` 이동 정보를 주소에 보존함; 선택 고객의 금액 메뉴로 이동했을 때 새 입금 매출·미수금 기준과 기존 청구·송장 기준이 다르면 기존 수치를 불러오거나 표시하지 않아 잘못된 고객 금액으로 오해하지 않게 함; 목록·상세의 로딩·데이터 없음·권한 없음·확인 필요·일부 조회·오류 상태와 사용자 문구를 자연스러운 한국어로 구분함 | Client 읽기 모델 23/23과 Analytics 전체 126/126, 실제 서명 HTTP와 고객별 ACL만 허용된 사용자를 포함한 Client 운영 API 10/10, 웹 모델·안전 어댑터 10/10, UI 계약 32/32, 실제 Playwright 고객 목록·4개 탭·부분 권한·권한 없음·비허용 ID·두 금액 메뉴의 기존 수치 차단·390px 가로 넘침 1/1, 웹 전체 189개 중 188개 통과·의도된 건너뜀 1개·실패 0개; web typecheck·production build·`git diff --check` PASS, 1440px·390px 최신 화면 수동 점검; AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W02-T02` | 완료 | `신규 고객`에서 개인과 법인·단체를 구분해 정식 이름, 개인 연락처 또는 법인 형태·선택 사업자등록번호, 선택 입금자명을 입력하고 쓰기 전에 같은 이름·등록번호 중복을 검토; 법인 형태는 서버에서도 필수로 검사해 Organization과 ClientGroup에 함께 저장하고, 개인 이메일·전화는 각각 정식 ContactPoint로 저장해 등록 직후 고객 상세에서 원문·내부 ContactPoint ID 없이 `이메일/전화 · 보호됨`으로 모두 확인함; 사용자에게 허용된 같은 이름 고객은 구분 확인 후에만 등록하고 권한 밖 후보가 있으면 이름·ID·건수를 숨긴 채 등록을 차단하며, 검토 확인값을 서명된 검토 사용자·당시 실제로 보인 후보 묶음·권한 판정에 묶어 후보나 접근 권한이 달라지면 등록 대신 재검토를 요구함; 서명 세션의 테넌트·사용자를 권위로 삼아 `Party`·`Entity`·`Person/Organization`·`ClientGroup`과 선택 `PartyIdentifier`·`PartyAlias(bank_depositor_name)`를 한 트랜잭션으로 저장하고 감사·outbox·사용자별 멱등성을 함께 보존; 새 요청은 항상 현재 중복·권한 검토를 통과해야 하지만 이미 성공한 같은 사용자·같은 요청키·같은 입력의 재시도는 이후 후보나 ACL이 바뀌어도 저장된 결과만 반환하고 다시 쓰지 않으며, 다른 입력의 요청키 재사용은 409로 차단함; 등록 실패 시 전체를 되돌리고, 활성 입금자 별칭만 다음 은행 입금의 정확 일치 후보로 사용하며 중복 별칭은 계속 `연결 확인 필요`로 유지함 | Master Data 등록 서비스 11/11과 패키지 전체 109/109, 등록 API 11/11 및 서명 세션·권한·교차 테넌트·중복 정보 비노출 관련 API·역할 36/36, 고객 상세 연결 집중 24/24와 Analytics 전체 127/127, 실제 임시 PostgreSQL에서 고객 그래프·법인 형태·연락처·별칭·등록번호·감사·outbox·멱등성 저장 1/1, Finance 별칭 회귀 포함 전체 146/146, 신규 고객·고객 상세 웹 모델·안전 어댑터·실제 브라우저 집중 17/17과 웹 전체 195개 중 194개 통과·의도된 건너뜀 1개·실패 0개, typecheck·production build PASS; 1440px·390px 등록 흐름과 390px 가로 넘침 0, 연락처 2종의 보호 표시, 재시도 요청키 고정, 필드·후보·ACL 변경 시 새 등록 재검토와 성공 결과 무변경 재생, 권한/오류 상태를 확인했고 AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W02-T03` | 완료 | `새 문의`를 raw Lead 목록 대신 canonical `GET /api/crm/inquiries`와 허용 목록 안의 상세 조회에 연결해 Outlook 추가 기능·직접 등록 경로, 여섯 사용자 상태, 명시적 담당자, 다음 행동, 상담 기록과 안전한 메일 메타데이터를 표시하고 첫 문의를 임의로 선택하지 않음; 주소의 `inquiry_id`가 현재 허용 목록에 있을 때만 상세를 열고 선택·닫기·Escape 뒤 원래 행으로 초점을 돌리며 모달 안 Tab 순환을 유지; 표시 사본은 요청한 증거 ID·SHA-256·바이트 수·`text/plain` MIME·검사 상태가 모두 맞을 때만 실행 불가능한 평문으로 보여 주고, 원본은 같은 ID·SHA·크기·`message/rfc822` 계약을 통과한 뒤 사용자가 누른 경우에만 `.eml`로 내려받으며 격리·권한 없음·확인 필요·일부 조회·오류를 각각 구분; 문의를 닫거나 바꾸면 늦게 도착한 본문·원본 응답을 무효화해 닫힌 화면에서 다운로드나 상태 갱신이 일어나지 않음; 레코드 소유자는 담당자로 승격하지 않음 | 웹 모델·signed-session 어댑터·실제 Playwright 6/6, 문의 조회·Outlook 증거 backend 16/16, 독립 코드 재검토 42/42·typecheck·build 및 `APPROVE`, 웹 전체 211개 중 210개 통과·기존 Matter 환경 건너뜀 1개·실패 0개, UI 계약 32/32, web typecheck·production build·`git diff --check` PASS; 1440px 목록, 820px 상세, 390px 전체폭 상세와 사이드바 전환 완료 목록에서 가로 넘침 0 및 수동 점검; `.omo/evidence/client-inquiry/`에 화면·원본·검증 보고서 기록, AI slop review PASS; 실제 Graph mailbox·AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W02-T04` | 완료 | `수임 현황`을 `전체`·`검토 중`·`상담 연결 대기`·`상담 연결됨`·`수임 확정`·`수임하지 않음`의 canonical 상태 탭과 고객·요청 범위 검색으로 구성하고, 주소의 `opportunity_id`가 현재 탭·검색 결과의 허용 목록에 있을 때만 상세와 행동을 열어 첫 항목을 임의 선택하지 않음; 상담 연결은 선택한 Opportunity 한 건만 기존 Intake 인계 API로 보내고 Matter를 직접 만들지 않으며, 응답이 사라진 뒤 재시도해도 같은 요청키와 Intake ID를 쓰되 구두점이나 긴 공통 접두사가 다른 Opportunity는 서로 다른 전체값 기반 키를 사용함; 연결 성공으로 상태 탭이 바뀌어도 선택·결과·초점을 보존하고, 선택 변경 뒤 늦게 도착한 응답과 권한 없음·확인 필요·차단 응답은 목록을 낙관적으로 바꾸지 않음; 검색은 전역 `query`와 분리한 `opportunity_query`를 사용하고 상세 닫기·Escape 뒤 원래 행으로 초점을 돌리며 1440·820·390px에서 목록·상세·상태를 가로 넘침 없이 표시함 | 모델·API client·실제 Playwright 집중 10/10, Opportunity→Intake·수임 결정 backend 14/14, 독립 최종 재검토 `APPROVE`, T04 포함 웹 회귀 211개 중 210개 통과·기존 Matter 환경 건너뜀 1개·실패 0개, UI source 계약 2/2, web typecheck·production build·`git diff --check` PASS; `.omo/evidence/client-opportunity/`의 1440·820·390px 화면과 키 충돌·재시도 증거 수동 점검, AI slop UI·문구 PASS; AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W02-T05` | 완료 | `상담·수임 관리`를 서울 시간 기준 `오늘 상담`·`전체`·`예정`·`완료` 탭과 고객·상담 검색, 명시적 `consultation_id` 선택으로 구성하고 첫 상담을 임의로 선택하지 않음; 권한이 확인된 문의의 최신 Lead version으로만 상담을 예약하고 일정 변경·완료·Outlook 일정 연결 뒤 문의 상세를 다시 읽어 다음 수임 결정이 오래된 version을 보내지 않게 하며, 네트워크 실패 재시도에는 같은 요청키·기대 version·완료 시각을 사용함; 상담 완료 시 결과와 다음 행동을 읽기 전용으로 보여 주고 완료된 상담에서는 일정 변경·Outlook 연결·완료 버튼을 제거하며 비밀 상담은 원문 대신 보호 안내만 표시함; 수임 확정은 수임료 금액 또는 `금액 미정`의 명시 확인을 요구하고 수임하지 않음은 사유를 필수로 하며, 부분 실패는 서버 workflow version으로 실패 단계만 재처리하고 Matter 개설은 별도 단계로 유지함; `접촉 이력`은 허용된 문의 ID만 보내고 Party·Opportunity는 서버가 Lead에서 파생하며, 조회 권한과 Lead 객체 ACL을 쓰기 전에 다시 확인하고 올바른 값·틀린 값·알 수 없는 내부 ID 입력을 같은 검증 실패로 처리해 존재 여부를 노출하지 않음; 일반 메모는 `note` 필드만 허용하고 같은 요청을 한 건으로 재생하며 사용자 메모·기록 사유 원문은 감사·재처리 기록에 남기지 않음; 상담·접촉 이력의 불러오는 중·데이터 없음·권한 없음·확인 필요·일부 조회·오류를 구분하고 1440·820·390px에서 실제 선택 상세·결과·버튼·초점 복귀를 가로 넘침 없이 표시함 | T05 모델·실제 Playwright 7/7, T03/T04 연관 회귀 16/16, Lead 메모·상담 backend 집중 10/10과 결합 회귀 26/26 및 기존 G6 1/1, 독립 frontend·backend 최종 재검토 모두 `APPROVE`, web typecheck·production build·`git diff --check` PASS; `.omo/evidence/client-consultation/`의 실제 URL·viewport·overflow 영수증과 1440·820·390px surface·detail 화면 수동 점검, 완료 상담의 dead action 0건·비밀 결과 비노출·raw workflow code 비노출 확인, AI slop review PASS; 실제 Graph 일정·AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W03-T01` | 완료 | `입금 매출 내역`을 권한 선필터 목록·상세와 cursor 기반 이어보기, XLSX/PDF 미리보기·확정, 정확 일치 자동 분류, 수동 고객 연결·재연결·해제·입금자명 기억, 환불 원입금 연결로 구성하고 청구서·Matter 없이 은행 입금 기준 매출 흐름을 한 화면에서 처리; 모든 변경은 선택 거래·기대 version·멱등 요청키·단일 영수증에 묶고 실제 처리와 재처리 모두 현재 고객·직원·거래 권한을 다시 확인하며, 500건을 넘는 허용 행도 `[200, 200, 102]` cursor로 누락 없이 조회; 0건인 일부 조회를 데이터 없음으로 바꾸지 않고, 파일·계좌·선택 거래·version·행동이 바뀐 뒤 도착한 미리보기·확정·분류 응답을 폐기하되 같은 문맥의 재처리는 허용; 사전 인코딩 파일도 확장자·MIME·원본 byte 수·base64·XLSX 16MiB/PDF 8MiB를 동일하게 검사하고 비JSON 409·403·503의 실제 HTTP 상태를 보존하며, 목록 행의 기본 버튼 역할과 Escape 초점 복귀를 유지; 1440·820·390px에서 첫 거래 자동 선택 없이 필터·안전 상세·가져오기·연결·환불을 가로 넘침 없이 표시하고 기존 Client 청구 표를 입금 매출로 오인하지 않게 분리 | backend 24/24, Billing 160/160, G7 21/21, PostgreSQL 1/1과 추가 ACL·재처리·cursor 적대 검증 통과; 모델·API·adapter·브라우저 독립 focused 39/39, 인접 T03~T05 회귀 55/55, process-isolated 전체 web 288개 중 287개 통과·기존 skip 1개·실패 0개, typecheck·production build·diff-check PASS; 독립 backend·web 최종 재검토 모두 `APPROVE`, 독립 QA `PASS`; `.omo/evidence/cl-p5-w03-t01-client-deposit-backend-final/`, `.omo/evidence/client-deposit/`, `.omo/evidence/client-deposit-independent-qa/`에 영수증과 실제 화면 기록, 신규 입금 UI AI slop review PASS; AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W03-T02` | 완료 | canonical `GET /api/finance/client-receivables`가 서명 세션의 tenant·수임료 조회 권한과 ClientGroup 객체 ACL을 상태·이름·Finance 원천 정규화보다 먼저 적용하고, 허용 고객의 활성 수임료 약정·입금 분류·거래·배분만 읽어 고객별 약정액·배분액·미수금·금액 미입력·선입금·초과 입금을 계산; 고객 500건·원천별 5,000건·Finance 합계 10,000건을 넘으면 잘린 합계 대신 `확인 필요`로 차단하며 청구서·Matter·Opportunity와 은행 원문·계좌·메모·권한 밖 건수는 반환하지 않음; 웹은 이 단일 조회만 사용해 `전체`·`미수금 있음`·`금액 미정`·`선입금·초과 입금`·`정산 완료` 5개 탭과 명시적 고객·약정·입금 선택을 제공하고, 최초 약정은 상담·수임 관리의 수임 확정에서 만들도록 이 화면의 중복 등록 행동을 제거; 약정 수정·취소와 입금 재배분은 선택 레코드·기대 version·사유·안정 요청키에 묶고 응답의 고객·약정·입금·금액·version을 다시 대조한 뒤 반영하며, 선택 변경 뒤 늦게 끝난 변경도 canonical 목록을 새로 읽되 오래된 알림·초점은 버리고 취소된 약정은 다음 조회에서 목록에서 사라짐; 비JSON 409 상태와 권한 하향을 보존하고 로딩·데이터 없음·권한 없음·확인 필요·일부 조회·오류를 구분하며 1440·820·390px에서 키보드와 가로 넘침을 검증 | backend 6/6과 Billing 160/160·G7 21/21·입금 12/12·PostgreSQL 12/12, 모델·API·container·패널·실제 브라우저 집중 82/82 및 root 최종 집중 40/40, web typecheck·production build·`git diff --check` PASS; 독립 backend·web 코드 재검토 모두 `APPROVE`, 독립 수동 QA `PASS`; `.omo/evidence/client-ar-backend-read/`, `.omo/evidence/client-ar-web-integration/`, `.omo/evidence/client-ar-integration-independent-qa/`에 1440·820·390px 화면·충돌·race·정보 비노출 영수증 기록, AI slop review PASS; 전체 web 최종 회귀는 P6 통합 뒤 재실행하며 AWS·배포·운영 데이터 변경 없음 |
| `CL-P5-W03-T03` | 완료 | `월별 입금 매출`·`문의 현황`·`고객 매출 순위`·`미수금 순위` 4종을 같은 서명 snapshot으로 화면·CSV에 제공하고, 인쇄·기간 선택·갱신 시각·일부 조회 상태를 Client `리포트` 메뉴에 통합; 파일·PostgreSQL 객체 ACL 권위를 실제 세션에 연결하고 ClientGroup `resource_type`·전역 `*`·deny 우선·비정규 공백·희소 배열을 fail-closed로 처리해 내보내기가 화면 권한을 넓히지 않음 | backend·PostgreSQL·signed HTTP·모델·container·패널·실제 브라우저 집중 검증과 독립 적대 재검토 `WATCH / APPROVE`; 화면=CSV snapshot 일치, 1440·820·390px 인쇄·반응형·일부 조회 검증, 웹 전체 307개 중 306개 통과·기존 skip 1개·실패 0개 |
| `CL-P6-W01-T01` | 완료(로컬) | 모든 Client 집계·고정 리포트·CSV·MIME 조회에 원천 조회 전 권한검사와 객체 ACL을 적용하고, 감사·권한 축소·내보내기 비확장·교차 테넌트·민감 원문 공격을 fail-closed로 검증 | Client 보안·ACL·감사 집중 검증과 파일·PostgreSQL 적대 재검토 `WATCH / APPROVE`; 운영 서명 증거는 출시 게이트 |
| `CL-P6-W01-T02` | 완료(로컬) | signed session의 사용자별 기능 권한과 ClientGroup 객체 ACL을 결합하고, 파일·PostgreSQL 권위 resolver 및 정확 `resource_type`·전역 ACL·deny 우선·비정규 식별자 거절을 실제 API 경로에 연결 | 역할별 allow/deny 표, 403→200 우회 재현 방지, malformed ACL 503, sparse array·공백·다른 resource type 비노출 검증; 운영 allow/deny 영수증은 출시 게이트 |
| `CL-P6-W01-T03` | 완료(로컬) | Client 모델 카탈로그와 additive PostgreSQL migration, 복구·재실행·되돌리기, 파일/PostgreSQL 읽기 공급자와 API 전환을 구현 | disposable PostgreSQL migration·recovery·read-path 5/5, schema count/digest·멱등 재실행·부분 이력·rollback 검증; 운영 DB 이관은 미실행 |
| `CL-P6-W01-T04` | 완료(로컬) | 기존 정확/수동 분류와 새 규칙을 읽기 전용으로 대사하고, 접두어·모호한 입금자·수임료 확인 여부·누락 약정·권한 밖 자료를 자연스러운 한국어 사유와 JSON/CSV로 분리 | 집중 13/13, 관련 46/46, Billing 159/159, 독립 적대 재검토 `WATCH / APPROVE`; 합성 자료만 사용 |
| `CL-P6-W02-T01` | 완료(로컬) | 10개 Client 메뉴의 불러오는 중·데이터 없음·권한 없음·확인 필요·일부만 불러옴·오류를 서로 다른 한국어·의미 구조로 제공하고, 상세 닫기 후 초점 복원과 반응형 배치를 통합 검증 | 10개 메뉴 × 6상태 × 3 viewport = 180상태, overflow 180건, 키보드 초점 이동 13건과 해당 없음 168건, 제품 공백 0; 웹 전체 307개 중 실패 0 |
| `CL-P6-W02-T02` | 완료(로컬) / 출시 차단 | 은행 가져오기 실패·Graph 메일/일정 장애·수임 처리 복구·대시보드 일부 조회의 탐지·복구·되돌리기 절차와 실행 가능한 로컬 명령을 고정 | 4개 drill, 로컬 명령 8개, rollback 포함 전건 exit 0, runbook `contract_verdict=PASS`; 실제 Graph 장애 영수증 1건은 `BLOCKED_EXTERNAL` |
| `CL-P6-W02-T03` | 완료(검증 도구) / 출시 차단 | 32개 VC·집중 테스트·전체 회귀·web/add-in build·migration/API/package 증거를 결합하고, 외부 trust anchor·clean exact-main·패키지·API·로그인 화면·배포 영수증이 모두 같을 때만 출시 PASS하도록 검증기를 구현 | 핵심 VC·API·웹 이동·대사·패키지 결합 93/93, 핵심 API·PostgreSQL 권한 결합 43/43, package verifier 31/31, fixture 결합 65/65, 전체 저장소 5,141/5,141, 웹 307개 중 실패 0·기존 skip 1, add-in 5/5, web typecheck와 web/add-in production build PASS, 변조·PII·symlink·dirty tree·위조 서명 차단; 현재 exact-main=false·logged-in=false·deployed=false이므로 출시 verdict는 `BLOCKED_EXTERNAL` |
...
기존 인증·권한 회귀도 함께 실행해 49/49 통과했다. 이 수치는 P0 계약과 세션 권한 회귀만 뜻하며, 전체 53개 TUW 완료 수와는 별개다.
