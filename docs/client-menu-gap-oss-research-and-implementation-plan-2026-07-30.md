...
- 문서 상태: 구현 진행 중 — P0 기준 계약 5/5, P1 은행 입금 매출 7/7, P2 수임료·미수금 6/6, P3 Outlook 문의·상담·수임 12/12, P4 Client 운영 조회 5/5 완료, P5 Client 10개 메뉴 9/11 완료, 전체 44/53 완료
...
이 계획은 이전 33개 초안을 대체한다. v2 ID를 최초 실행 원장으로 사용하며, 2026-07-31 현재 P0의 5개 TUW, P1의 7개 TUW, P2의 6개 TUW, P3의 M365 연결·원본 메일 증거·문의 등록·Outlook 추가 기능·문의 조회·상담·수임 결정·Intake 인계 12개 TUW, P4의 고객 권한 선필터·5개 KPI·오늘 확인할 일·12개월 추이·고객 순위·통합 대시보드 API 5개 TUW와 P5의 Client 메뉴·직접 주소, 대시보드 KPI·오늘 확인할 일, 월별 입금 매출·문의 현황·고객 순위, 고객 목록·상세, 신규 고객 등록, 새 문의 목록·상세, 수임 현황 7개 TUW를 구현·집중 검증했다.
...
| `CL-P5-W03-T02` | 진행 중 | 수임료 약정·입금 배분·미수금·금액 미입력·선입금·초과 입금 모델과 5개 상태 탭, 명시적 고객·약정·입금 선택, version 충돌·키보드·1440/820/390 패널은 독립 검증 완료; 실제 Client 화면 연결에 필요한 canonical `GET /api/finance/client-receivables` 읽기 경로가 없음을 확인해 권한 선필터·감사·부분 조회 계약과 API adapter/container를 구현 중 | 독립 모델 19/19·패널 8/8 및 최종 재검토 `APPROVE`; 수임 확정 workflow가 최초 FeeCommitment를 생성하므로 raw CRM Opportunity 선택이나 청구서·Matter 의존 없이 조회·수정·취소·재배분 end-to-end 연결을 완료해야 함 |
