...
- 문서 상태: 구현 진행 중 — P0 기준 계약 5/5, P1 은행 입금 매출 7/7, P2 수임료·미수금 6/6, P3 Outlook 문의·상담·수임 12/12, P4 Client 운영 조회 5/5 완료, P5 Client 10개 메뉴 10/11 완료, 전체 45/53 완료
...
이 계획은 이전 33개 초안을 대체한다. v2 ID를 최초 실행 원장으로 사용하며, 2026-07-31 현재 P0의 5개 TUW, P1의 7개 TUW, P2의 6개 TUW, P3의 M365 연결·원본 메일 증거·문의 등록·Outlook 추가 기능·문의 조회·상담·수임 결정·Intake 인계 12개 TUW, P4의 고객 권한 선필터·5개 KPI·오늘 확인할 일·12개월 추이·고객 순위·통합 대시보드 API 5개 TUW와 P5의 Client 메뉴·직접 주소, 대시보드 KPI·오늘 확인할 일, 월별 입금 매출·문의 현황·고객 순위, 고객 목록·상세, 신규 고객 등록, 새 문의 목록·상세, 수임 현황, 상담·수임 관리·접촉 이력, 입금 매출 내역, 수임료·미수금 10개 TUW를 구현·집중 검증했다.
...
| `CL-P5-W03-T03` | 진행 중 | 고정 리포트 4종의 snapshot 기반 조회·CSV 계약과 화면 모델·패널·독립 container를 구현하고, Client `리포트` 메뉴의 기존 범용 보고서 화면을 이 전용 경로로 교체하는 통합 작업 진행 중 | backend 집중 7/7·인접 회귀 24/24·PostgreSQL 10/10, 모델·패널 17/17과 독립 코드 재검토 `APPROVE`; 화면=CSV snapshot 일치, 내보내기 권한 비확장, 인쇄·1440/820/390·공통 상태 계약을 실제 Client 메뉴 통합 후 최종 검증해야 함 |
