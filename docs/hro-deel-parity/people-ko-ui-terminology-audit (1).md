...
Status: current terminology audit  
Date: 2026-06-24  
...
| 수정 필요 후보 | `아티팩트` fallback, `온보딩/오프보딩`, `오퍼` |
...
| 문서 | 유지 가능 | 패널 제목은 `인사 문서`라 충분함. 메뉴도 더 명확히 하려면 `인사 문서` 가능 |
...
| 정책 | 유지 가능 | 패널 제목은 `인사 정책`이라 충분함. 메뉴도 `인사 정책` 가능 |
| 감사 | 유지 가능 | 보안/감사 맥락에서는 자연스러움. 일반 사용자 화면이면 `활동 기록` 권장 |
| 현황 | 유지 가능 | 더 명확히 하려면 `인사 현황` |
...
- Recommended terms:
  - Stage: `채용 제안`
  - Document: `채용 제안서`
- Status: 제품 톤 선택 필요. 한국어 우선이면 수정 권장

### P3 - `문서`, `정책`, `현황`, `감사` 메뉴는 간결하지만 더 구체화 가능

- File: `apps/web/src/components/Shell.jsx`
- Current: `문서`, `정책`, `현황`, `감사`
- Recommended if more HR-specific:
  - `문서` -> `인사 문서`
  - `정책` -> `인사 정책`
  - `현황` -> `인사 현황`
  - `감사` -> `활동 기록`
- Status: 필수 수정 아님. 메뉴 폭과 정보밀도 기준으로 결정
...
3. 제품 톤을 한국어 우선으로 확정하면 `오퍼`/`오퍼 문서` -> `채용 제안`/`채용 제안서`
