...
- `Client` is user-facing `고객`.
- `고객` includes individuals and organizations.
...
This plan maps that pattern to Korean legal-operations language: `고객`, `담당자`, `Opportunity`, `상담·문의`, `접촉 이력`, `제안·계약`, `고객 관계`, `이해상충 확인`, `청구·수금`, `고객 리포트`, and `고객 설정`.
...
| CLIENT-UI-T02 | Navigation | completed | `apps/web/src/data/nav.js`; `apps/web/src/i18n.js` | Top nav/search labels show `고객` |
| CLIENT-UI-T03 | Capability Copy | completed | `apps/web/src/data/capabilityMap.js` | Capability label and boundary use `고객` |
| CLIENT-UI-T04 | Page Header | completed | `apps/web/src/components/ClientsSurface.jsx` | Header title/subtitle use `고객`, `담당자`, `Opportunity`, `상담 이력` |
...
| CLIENT-UI-T10 | Right Detail Panel | completed | `ClientsSurface.jsx` | Right panel metrics use `고객`, `잠재 고객`, `담당자`, `Opportunity` |
| CLIENT-UI-T11 | State Copy | completed | `ClientsSurface.jsx` | Empty/error/denied/review states use `고객` vocabulary |
...
| CLIENT-UI-T13 | Tests | completed | `apps/web/test/ui-regression.test.mjs`; `scripts/verify-matter-ui-flows.mjs` | Tests assert `고객` and current route markers |
...
- For visible Client UI copy, use `고객`.
...
Do:
- Change `navItems` user label for `clients` from `Client` to `고객`.
- Change `copy.ko.clients`, `copy.ko.clientsTitle`, `copy.en.clients`, and `copy.en.clientsTitle` to `고객` because the current app serves Korean UI in both locale maps.

Check:
- Source assertions in UI tests find `고객`.
- Browser top axis shows `고객`.
...
Do:
- Change capability `label` for `client` to `고객`.
- Change boundary to `고객 정보와 연결된 Matter를 권한 범위 안에서 확인합니다.`
...
Do:
- Title comes from `labels.clientsTitle` as `고객`.
- Subtitle becomes `고객, 담당자, Opportunity, 상담 이력을 한 화면에서 확인합니다.`
...
- `client-accounts`: current CRM account table as `법인·개인 고객`.
- `client-data`: current data enrichment as `고객 데이터`.
- `client-reports`: current report builder as `고객 리포트`.
- `client-import`: current import mapping as `고객 데이터 가져오기`.
...
Do:
- Header eyebrow `레코드` -> `고객 정보`.
- Fallback title `Client` -> `고객`.
- `잠재 Client` -> `잠재 고객`.
- `기회` -> `Opportunity`.
- `접수` -> `상담·문의`.
- `계정` -> `고객`.
- `연락처` -> `담당자`.
- `데이터 보강 상태는 Client 데이터 관리...` -> `고객 데이터...`.
- `Client 보고서` -> `고객 리포트`.
...
- `검토가 필요한 Client가 있습니다.` -> `검토가 필요한 고객이 있습니다.`
...
Do:
- `Client 표시 이름` -> `고객 표시 이름`.
- `Client 작업 검토` -> `고객 작업 검토`.
- `Client 계정 레코드` -> `고객 정보`.
- `Client 레코드` -> `고객 정보`.
- `Client 연락처` -> `고객 담당자`.
...
- `apps/web/src/data/nav.js`, `apps/web/src/i18n.js`, `apps/web/src/data/capabilityMap.js`: top-level Client axis now displays as `고객`.
- `apps/web/src/components/Shell.jsx`: Client sidebar now exposes grouped Korean menu items for `고객 홈`, `고객 목록`, `법인·개인 고객`, `담당자`, `Opportunity`, `상담·문의`, `접촉 이력`, `제안·계약`, `고객 관계`, `이해상충 확인`, `청구·수금`, `고객 리포트`, `고객 데이터`, `고객 데이터 가져오기`, and `고객 설정`.
...
- `apps/web/src/components/DataCloudEnrichmentPanel.jsx`, `apps/web/src/components/ReportBuilderPanel.jsx`, `apps/web/src/data/apiClient.js`: customer data/report labels use `고객` while internal `Client` object contracts remain stable.
