# mater Desktop Pilot Install and Session Evidence
...
| Area | Status | Receipt |
| --- | --- | --- |
| Install | Pass | `apps/desktop/dist/mac/mater.app` exists after `npm --workspace apps/desktop run build:mac`. |
| Code signature | Pass | `codesign --verify --deep --strict --verbose=2 apps/desktop/dist/mac/mater.app` returned valid on disk and satisfies its Designated Requirement. |
| Launch | Pass | `apps/desktop/dist/mac/mater.app/Contents/MacOS/mater` returned `mater desktop internal macOS build 0.1.0`. |
