...
| MDT-P1-W01-T04 | complete | `scripts/validate-matter-desktop-security.mjs` |
...
- Added `scripts/validate-matter-desktop-security.mjs`.

Check:

```bash
node --check scripts/validate-matter-desktop-security.mjs
node scripts/validate-matter-desktop-security.mjs
git diff --check -- scripts/validate-matter-desktop-security.mjs
```
...
- Display `matter by AMIC` before main renderer readiness.
...
```bash
rg -n "splash|matter by AMIC|ready-to-show" apps/desktop/src/main
node -e "const m = await import('./apps/desktop/src/main/splash.js'); if (!m.splashHtml().includes('matter by AMIC')) process.exit(1); if (!m.splashDataUrl().startsWith('data:text/html')) process.exit(1); console.log(m.SPLASH_BRAND);"
git diff --check -- apps/desktop/src/main/splash.js
```

Results:

- Splash source grep passed for `splash`, `matter by AMIC`, and `ready-to-show`.
- Node import check confirmed `matter by AMIC` and a data URL splash document.
- `git diff --check` passed.
