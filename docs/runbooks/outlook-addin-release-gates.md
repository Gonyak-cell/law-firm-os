...
- a protected pilot-assignment proof with opaque group references plus exact opaque principal-reference sets for the nine eligible Entra object identities and the one explicitly excluded identity; the validator recomputes both fingerprints and the `9/1` counts, rejects any eligible/excluded set intersection, and keeps the principal references out of the Git-safe receipt and command output; it also requires no tenant-wide assignment, ProductId overlap count `0`, and per-ProductId state proving Matter/full assigned while inquiry-only has zero group references and zero assignments;
