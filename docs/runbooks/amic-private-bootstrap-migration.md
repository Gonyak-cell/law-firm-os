...
The PostgreSQL API uses a separate S3 prefix for member photos within the
existing DMS bucket. Configure only `LAWOS_MEMBER_PHOTO_S3_PREFIX` from the
approved `photo_prefix`. The reader reuses these existing DMS bindings:

| Environment variable | Value source |
| --- | --- |
| `LAWOS_DMS_S3_BUCKET` | Must equal approved `photo_bucket_name` |
| `LAWOS_DMS_S3_EXPECTED_BUCKET_OWNER` | Must equal approved `photo_expected_bucket_owner` |
| `LAWOS_DMS_S3_REGION` | Must equal approved `aws_region` |
| `LAWOS_DMS_S3_KMS_KEY_ID` | Must equal approved `photo_kms_key_arn` |
| `LAWOS_DMS_S3_CREDENTIAL_REF` | Existing deployed API reader role reference, never credentials |
...
bucket owner. Duplicate `LAWOS_MEMBER_PHOTO_S3_*` bindings other than the prefix
are rejected. Missing or partial configuration fails startup; the API does not
...
configuration. `LAWOS_DMS_S3_PREFIX` continues to configure Vault and other DMS
users. Before creating a change set, resolve the complete Lambda environment
and validate its compact JSON UTF-8 size against the 4,096-byte limit. This
includes the variable names, values and serialization overhead measured by
the service; template string lengths and key-plus-value sums alone are not a
passing deployment check.
