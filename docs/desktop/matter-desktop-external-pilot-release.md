...
The verifier accepts no trust-root, registry-path, registry-digest, or environment-variable override. Its production path uses only the versioned governance-installed trust-root policy and fixed `import.meta.url`-rooted paths under `config/external-release/`. That policy must pin the canonical Ed25519 public-SPKI DER digest, exact registry SHA-256, exact raw 64-byte registry-signature SHA-256, and registry serial. It rejects private-key PEM input, symlinks/path escape, a root key reused as a receipt leaf, duplicate leaf SPKIs, digest/signature tampering, and serial replay; it parses only the exact registry and receipt byte snapshots that were hashed and signed. The current source policy is intentionally unconfigured, so production preparation and verification fail with `TRUST_ROOT_NOT_CONFIGURED` until the governance owner installs real policy pins and files outside the bundle and caller inputs. `trust-registry-reference.json` is signed reference data only; it cannot make a caller-created key trusted. Tests may inject a synthetic policy only through the explicit JavaScript API while `NODE_ENV=test`; neither command-line tool exposes that API.
