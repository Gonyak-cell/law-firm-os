...
approve their own deployment, and still forbid administrator bypass. This is
single-owner internal approval, not independent human review. The separate
read-only OIDC role and exact-version artifact readback remain mandatory.
The formal signed/public Windows release lane is unchanged.

Tasks:

- Use GitHub OIDC and a protected environment; no long-lived AWS keys.
- Create and protect both named GitHub environments before any workflow run; [GitHub may otherwise create a referenced missing environment](https://docs.github.com/en/actions/how-tos/deploy/configure-and-manage-deployments/manage-environments) without the intended rules. Require protected `main`, exactly the pinned repository owner as the sole `User` reviewer, and `prevent_self_review=false` before enabling any AWS role. Owner approval remains a required manual deployment step.
- Make each called job independently GET its exact environment definition with the short-lived repository `GITHUB_TOKEN` and only `actions: read` before dependency installation or AWS credential configuration. Require exactly one individual reviewer whose immutable GitHub ID matches the existing OIDC owner binding, self-review allowed, administrator bypass disabled, protected branches only, and no custom branch policy; reject replacement, extra, team, or missing reviewers. Emit a v2 receipt identifying `approval_mode=single-owner` without returning the reviewer identity or token.
...
| Server migration | Existing PostgreSQL identity/HRX ledgers and S3 photo port reused; fixed-account/role/region/stack read-only target discovery, signed execution packet, exact production-target binding, owner approval, local re-preflight, AWS caller/S3/KMS gates, Secrets Manager resolution, TLS PostgreSQL readiness, versioned photo commit, idempotent identity/HRX import, checkpoints, and safe repair receipt are implemented locally. Live readback proves the deployed production stack predates the explicit provider-ready parameters/output, so discovery correctly remains closed instead of treating absence as `false`. The W15 upgrade path now sends and revalidates six exact closed values: three for external providers and three for Outlook. It rejects every disabled provider/Outlook resource change, and post-execution verification requires the same closed parameters plus an explicit `ExternalReadProvidersEnabled=false` output. All 12 raw-template additions are condition-gated, and the production password-reset schedule remains at its deployed five-minute interval | Approved legal-entity mapping; separately authorize exact change-set creation and review, then execution, target discovery/private-network execution, and hosted G3 receipt |
...
| Private S3/CloudFront distribution | IaC policy, OAC/KMS/logging boundaries, decrypt-only CloudFront KMS authority, conditionally absent OIDC/runtime roles while disabled, one-time nine-object baseline, exact control-history readback, conditional control commits, monotonic revocations, idle-period metadata renewal, exact-version successor publisher, and isolated readback implemented locally; no stack provisioned | Single-owner protected environments verified on 2026-09-04; next authorized disabled-stack inspection, enablement, and G6 baseline/successor anonymous-denial and exact-VersionId receipts |
| OIDC publish pipeline | One manual dispatcher with no runner steps or direct AWS role trust explicitly selects baseline or successor and invokes separate protected reusable publisher and reader workflows; actions are SHA-pinned, immutable repository/owner IDs, each exact `job_workflow_ref`, and the exact caller `workflow_ref` are pinned, job-wide OIDC authority and non-secret dispatch inputs are documented, input documents and nested predecessor/bindings reject extra fields, a second baseline is rejected before writes, the first successor is anchored to the baseline marker, later successors are anchored to the latest channel pointer, and the channel pointer moves conditionally only after the 17 current objects plus predecessor readback. Both called jobs now use only `actions: read` on their short-lived repository token to verify the exact protected-environment rules before dependencies or AWS credentials; the shared v2 guard requires the exact immutable owner ID as the sole individual reviewer, permits self-review, and retains no-bypass/branch protections. Both live environment reads passed this single-owner policy on 2026-09-04 | Both environment gates passed; next authorize role enablement and run the authorized baseline, first and repeated successors, and capture G7 OIDC receipts |
...
- The production PostgreSQL template independently returned `PASS` with 86 resources, provider activation disabled by default, and canonical template SHA-256 `5a6299b062b38540fa8961a0b6ba51bc8c0ade3036dc445d87c51d80d522835d`; this is local source evidence only and was not deployed.
...
- The current full-repository run used the repository-required Node 22.22.3 and completed 6,608 tests with 6,607 passed, zero failed, and one explicit skip. This latest green run includes the short-lived-token GitHub environment guard, external-read readiness inspector, and exact disabled-provider/Outlook W15 promotion guards; the focused scope-isolation, W15, readiness, and private-bootstrap batch passed 59/59, and the focused internal-distribution/readiness batch passed 27/27. The earlier focused production/W15/readiness batch passed 26/26, private-bootstrap batch passed 34/34, and affected internal-distribution/Windows batch passed 60/60. A diagnostic run whose top-level runner was Node 22 but whose child processes resolved Node 26 produced the expected environment-gate failures; rerunning with Node 22 fixed for both parent and child processes proved those were runtime-version mismatches rather than product regressions. A prior pass correctly detected that the four Client Operations drill receipts still bound the pre-change `apps/api/src/server.js` hash; the four synthetic drills were actually recaptured, their contract returned `PASS` while Graph/AWS stayed `BLOCKED_EXTERNAL`, the focused contract passed 10/10, and the complete suite then finished green. Test-generated changes to 11 tracked manual-QA receipts were restored byte-for-byte to the source baseline after the latest run. The earlier unrelated lock regression was traced to concurrent fixture registration before the lock scenario; fixture setup is now sequential while the sweep/revoke concurrency, required serialization retry, and deadlock rejection remain unchanged. The email-DMS package then passed 377/377 and the focused lock scenario passed ten consecutive runs before the full green rerun.
...
- A second read-only comparison bound the deployed `lawos-production` template to the current 86-resource candidate without creating a change set. The raw template still adds 12 definitions: `ExternalReadSecretsPolicy`, `MicrosoftEgressBrokerLambdaEndpoint`, and the ten Outlook conversation-worker function/schedule/log/DLQ/alarm resources. It changes three existing resources (`ApiExecutionRole`, `ApiFunction`, and `SecretsManagerEndpoint`), removes none, and adds six parameters. Every one of the 12 added definitions now has an explicit false-by-default condition. The W15 reviewer was narrowed to projection additions and supporting modifications, requires all six provider/Outlook disabled values as exact non-inherited change-set parameters, and rejects any `ExternalReadSecretsPolicy`, Microsoft-egress, or Outlook-worker resource change while closed. No change set was created or executed, so only an authorized AWS change-set review can establish the parameter-resolved resource set and replacement behavior.
- Property-level static review confirms that `ExternalReadSecretsPolicy` is conditioned on `ExternalReadProvidersEnabled`; the Microsoft egress endpoint is conditioned on real Outlook configuration; and all ten worker resources and both outputs are conditioned on `OutlookConversationWorkerProvisioned`. The six Outlook API environment entries, two runtime-secret references, log scope, source-function deny scope, broker invocation, and DLQ send permission all resolve to `AWS::NoValue` when their matching Outlook condition is false. `OutlookConversationWorkerEnabled` controls runtime state separately, so temporarily disabling production traffic does not delete a provisioned queue or log group. The deployed and candidate password-reset schedules both remain `rate(5 minutes)`. This proves the source-side fail-closed contract, not a deployed stack update or a no-replacement AWS claim.
- The reusable external-read readiness inspector passed 4/4 focused tests. It requires a clean exact source root, fixes the read-only auditor account/role/region/stack, performs exactly three AWS reads, records only structural logical IDs and hashes in a private create-new receipt, and has no change-set, deployment, secret-value, or object-write path. Only an exact candidate-template match with the provider parameter and output both explicitly `false` returns `READY_DISABLED`; the current pre-upgrade state remains `UPGRADE_REVIEW_REQUIRED` by design.
...
- Historical GitHub read-only preflight before the 2026-09-04 single-owner decision found zero configured Actions environments, repository variables, or repository secrets. Actions are enabled with `allowed_actions=all`, repository-level SHA pin enforcement is off, and the default workflow token is read-only; this workflow therefore pins every used action itself. `main` requires a current HRX check, conversation resolution, and admin enforcement, and forbids force-push/deletion, but requires zero approving reviews. Absence of environments is not a protection mechanism because a referenced environment can be created by a workflow without the intended rules. Both new exact-name guard invocations returned the sanitized `BLOCKED` result against the live repository with exit code 2 and no token or reviewer identity output. The AWS roles are conditionally absent while the stack is disabled, and both protected environments must be created and verified before authorized enablement. Existing PR #330 and the unsigned 0.1.32 Draft Release were not changed.

### 2026-09-04 single-owner approval policy verification

- The owner explicitly withdrew the independent human reviewer requirement.
  Exactly the same pinned owner now approves both internal-unsigned environments,
  including their own runs. Manual approval is not removed.
- Both exact GitHub environments were configured and independently read back
  with the shared v2 guard: `PASS`, `approval_mode=single-owner`, one verified
  owner reviewer, `prevent_self_review=false`, `admins_can_bypass=false`, and
  protected branches only. Neither the raw reviewer identity nor the token is
  emitted by these receipts.
- The affected internal-distribution and formal-Windows policy regression
  passed 73/73 tests, including rejection of a wrong, extra, team, or missing
  owner reviewer and preservation of the formal lane's existing protections.
- This resolves the missing second-reviewer/environment prerequisite only.
  No AWS distribution role was enabled, no installer was published, and no
  Windows or hosted-data completion claim was added by this policy change.
- The formal signed/public Windows environment policies remain unchanged;
  separate read-only artifact verification, exact-main OIDC bindings, metadata
  signatures, privacy checks, and rollback/revocation gates still apply.
