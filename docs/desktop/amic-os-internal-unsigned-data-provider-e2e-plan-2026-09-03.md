...
- A second read-only comparison bound the deployed `lawos-production` template to the current 86-resource candidate without creating a change set. The candidate adds 12 logical resources: `ExternalReadSecretsPolicy`, `MicrosoftEgressBrokerLambdaEndpoint`, and the ten Outlook conversation-worker function/schedule/log/DLQ/alarm resources. It changes four existing resources: `ApiExecutionRole`, `ApiFunction`, `PasswordResetWorkerSchedule`, and `SecretsManagerEndpoint`; it removes none and adds six parameters, including the three provider controls. The general UPDATE reviewer still rejects every resource addition. The existing W15 exact-template path uses its fixed projection/Outlook resource allowlists, now including the conditional `ExternalReadSecretsPolicy`, while requiring the described change set to carry `EnableExternalReadProviders=false` and both exact disabled placeholders; missing values, `UsePreviousValue`, mismatches, changes outside those allowlists, and unsafe replacements fail closed. The runner supplies those three values explicitly and post-execution stack verification requires the same parameters plus `ExternalReadProvidersEnabled=false`. Because the allowlist intentionally covers the mixed Outlook/projection rollout surface, it does not by itself prove that this particular provider-readiness update is single-purpose. No change set was created or executed, so the live stack remains pre-upgrade and a production write still requires separate approval and review.
- A property-level static review of the same templates confirmed that `ExternalReadSecretsPolicy` is conditioned on `ExternalReadProvidersEnabled`, provider activation defaults to `false`, and force deletion is denied. It also found a mixed-scope production delta: the Microsoft egress endpoint and ten Outlook worker resources are not conditionally omitted when the worker is disabled, the API role gains exact Lambda-invoke and SQS-dead-letter actions plus two additional runtime-secret resource forms, ten API environment keys are added, and the existing password-reset schedule changes from `rate(5 minutes)` to `rate(1 minute)`. The Outlook schedule itself defaults disabled, but those resources would still be present. This static receipt is not a parameter-resolved CloudFormation change set and therefore authorizes neither deployment nor a no-replacement claim; the unrelated delta must be isolated or explicitly reviewed as mixed scope.
