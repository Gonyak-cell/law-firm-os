...
- A second read-only comparison bound the deployed `lawos-production` template to the current 86-resource candidate without creating a change set. The candidate adds 12 logical resources: `ExternalReadSecretsPolicy`, `MicrosoftEgressBrokerLambdaEndpoint`, and the ten Outlook conversation-worker function/schedule/log/DLQ/alarm resources. It changes four existing resources: `ApiExecutionRole`, `ApiFunction`, `PasswordResetWorkerSchedule`, and `SecretsManagerEndpoint`; it removes none and adds six parameters, including the three provider controls. The general UPDATE reviewer still rejects every resource addition. The existing W15 exact-template path has now been tightened to admit the conditional `ExternalReadSecretsPolicy` addition only while the described change set carries `EnableExternalReadProviders=false` and both exact disabled placeholders; missing values, `UsePreviousValue`, mismatches, unsafe replacements, and unrelated resource changes all fail closed. The runner supplies those three values explicitly and post-execution stack verification requires the same parameters plus `ExternalReadProvidersEnabled=false`. No change set was created or executed, so the live stack remains pre-upgrade and a production write still requires separate approval and review.
