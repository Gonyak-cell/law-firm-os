...
| --- | --- | --- | --- | --- |
| matter | `mater://matter/:matterId` | `view=matters` with selected matter context | Recheck matter membership and Matter Core read permission before entry. | denied |
| document | `mater://document/:documentId` | `view=vault` with selected document context | Recheck document ACL, matter membership, and Vault read permission before entry. | denied |
| task | `mater://task/:taskId` | `view=home` or future task panel | Recheck task assignment and matter membership before entry. | denied |
| auth | `mater://auth/callback` | auth callback handler only | Validate PKCE state, issuer, and nonce in main process. | fallback |
